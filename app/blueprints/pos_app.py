...
QR_SECRET = os.getenv("QR_SECRET") or os.getenv("SECRET_KEY", "pos-default-secret-key-change-me")
...
# 初期設定
# -----------------------------------------------------------------------------
# POSシステム設定フラグ（ブループリント登録後にapp.configに設定）
POS_ALLOW_DIRECT_REFUND = False
POS_DEBUG_TOTALS = True
POS_DEBUG_CANCEL = True
...
def get_db_session():
    ...
    try:
        ...
        try:
            session.rollback()
            current_app.logger.error(f"[DB] Transaction rolled back due to error: {e}")
        except Exception as rollback_error:
            current_app.logger.error(f"[DB] Rollback failed: {rollback_error}")
        raise
    finally:
        # セッションをクローズ
        try:
            ...
            if hasattr(SessionLocal, 'remove'):
                ...
                if exception is not None:
                    try:
                        ...
                        session = SessionLocal()
                        session.rollback()
                        current_app.logger.info(f"[teardown] rolled back session due to exception: {exception}")
                    except Exception as rollback_error:
                        current_app.logger.error(f"[teardown] rollback failed: {rollback_error}")
                # セッションをクリーンアップ
                SessionLocal.remove()
        # db-per-tenant モードの場合
            ...
            if hasattr(g, 'tenant') and g.tenant:
                try:
                    ...
                    current_app.logger.error(f"[teardown] tenant session cleanup failed: {tenant_error}")
    except Exception as e:
        current_app.logger.error(f"[teardown] session cleanup failed: {e}")
...
def ensure_store_id_in_master(store_code: str, store_name: str = None) -> int:
    ...
    try:
        ...
        current_app.logger.info(f"店舗IDマスターに新規登録: 店舗ID={new_store.店舗ID}, 店舗コード={store_code}")
        return new_store.店舗ID
        
    except Exception as e:
        s.rollback()
        current_app.logger.error(f"店舗IDマスター登録エラー: {e}")
        raise
    finally:
        s.close()
        SessionLocal.remove()
...
def validate_store_id(store_id: int) -> bool:
    s = SessionLocal()
    try:
        current_app.logger.info("[validate_store_id] lookup 店舗ID=%r", store_id)
        store = s.query(M_店舗IDマスター).filter(
            M_店舗IDマスター.店舗ID == store_id,
            M_店舗IDマスター.有効フラグ == 1
        ).first()
        current_app.logger.info("[validate_store_id] result=%s", bool(store))
...
def _auto_set_store_id(session_obj, flush_context, instances):
    ...
    try:
        ...
        try:
            current_app.logger.warning("[auto_set_store_id] store_id not int: %r", sid)
        except Exception:
            pass
        return
    ...
    try:
        ...
        try:
            current_app.logger.debug("[auto_set_store_id] new_objs=%d sid=%s", len(new_objs), sid)
        except Exception:
            pass
    except Exception:
        new_objs = list(session_obj.new)
    ...
    for obj in new_objs:
        ...
        if cur in (None, "", 0):
            try:
                ...
                try:
                    current_app.logger.debug("[auto_set_store_id] set store_id=%s for %s",
                ...
                try:
                    import traceback
                    current_app.logger.error("[auto_set_store_id] set failed for %s: %s\n%s",
...
class OrderItem(TenantScoped, Base):
    ...
    kitchen_printed = Column("厨房印刷済み", Boolean, nullable=False, default=False)  # 厨房プリンターで印刷済みかどうか
    receipt_printed = Column("明細印刷済み", Boolean, nullable=False, default=False)  # 明細（レシート）プリンターで印刷済みかどうか
    store = relationship("Store")
...
# --- [モデル] プリンタ印刷設定（PrinterFormatSetting） ----------------------------------------
class PrinterFormatSetting(TenantScoped, Base):
    """プリンタ印刷設定モデル"""
    __tablename__ = "M_プリンタ印刷設定"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    store_id = Column("店舗ID", Integer, ForeignKey("M_店舗.id", ondelete="CASCADE"), nullable=False)
    printer_name = Column("プリンタ名", Text, nullable=False)
    
    # 印刷モード
    print_mode = Column("印刷モード", Text, nullable=False, default='kitchen')
    
    # 余白設定
    margin_before = Column("印刷前余白", Integer, nullable=False, default=1)
    margin_after = Column("印刷後余白", Integer, nullable=False, default=1)
    
    # 伝票モード専用
    show_title = Column("タイトル表示", Integer, nullable=False, default=1)
    title_text = Column("タイトルテキスト", Text, nullable=False, default='ご注文内容')
    title_size = Column("タイトルサイズ", Integer, nullable=False, default=2)
    title_align = Column("タイトル揃え", Text, nullable=False, default='center')
    title_bold = Column("タイトル太字", Integer, nullable=False, default=1)
    show_separator = Column("区切り線表示", Integer, nullable=False, default=1)
    show_datetime = Column("日時表示", Integer, nullable=False, default=1)
    datetime_format = Column("日時フォーマット", Text, nullable=False, default='YYYY/MM/DD HH:mm:ss')
    show_price = Column("金額表示", Integer, nullable=False, default=1)
    
    # 厨房モード専用
    show_new_additional = Column("新規追加表示", Integer, nullable=False, default=1)
    kitchen_datetime_format = Column("厨房日時フォーマット", Text, nullable=False, default='HH:mm')
    
    created_at = Column("作成日時", DateTime, default=datetime.utcnow)
    updated_at = Column("更新日時", DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
def ensure_kds_category_tables():
    with engine.begin() as conn:
        ...
        try:
            current_current_app.logger.info(
...
def migrate_schema_if_needed():
    ...
    with eng.begin() as conn:
        # T_注文明細の不足カラム（厨房/明細印刷済みフラグ）
        if "T_注文明細" in tables:
            cols = {c["name"] for c in insp.get_columns("T_注文明細")}
            if "厨房印刷済み" not in cols:
                conn.exec_driver_sql('ALTER TABLE "T_注文明細" ADD COLUMN "厨房印刷済み" BOOLEAN NOT NULL DEFAULT false')
            if "明細印刷済み" not in cols:
                conn.exec_driver_sql('ALTER TABLE "T_注文明細" ADD COLUMN "明細印刷済み" BOOLEAN NOT NULL DEFAULT false')
            ...
            if t not in tables:
                ...
                if tbl is not None:
                    try:
                        ...
                        try:
                            current_current_app.logger.info(f"created missing core table: {t}")
                        except Exception:
                            pass
                    except Exception as _e:
                        try:
                            current_current_app.logger.warning(f"create table {t} failed: {_e}")
                        except Exception:
                            pass

        # (デバッグ)今の metadata に載っているテーブル名を一度だけログ
        try:
            current_current_app.logger.info(f"metadata tables = {list(Base.metadata.tables.keys())}")
        except Exception:
            pass
    ...
    try:
        ...
        try:
            current_current_app.logger.info("auto_add_missing_columns completed successfully")
        except Exception:
            pass
    except Exception as e:
        try:
            current_current_app.logger.warning(f"auto_add_missing_columns failed: {e}")
        except Exception:
            pass
...
try:
    ...
    try:
        current_current_app.logger.info(f"DB url = {engine.url}")
        if engine.url.database:
            import os
            current_current_app.logger.info(f"DB file = {os.path.abspath(engine.url.database)}")
    except Exception:
        pass

except Exception as e:
    current_app.logger.warning(f"Schema init warning: {e}")
        ...
        for column in table.columns:
            ...
            if column.name not in db_columns:
                ...
                try:
                    ...
                    try:
                        current_current_app.logger.warning(f"[auto_add_missing_columns] Failed to add column {table_name}.{col_name}: {e}")
                    except Exception:
                        pass
...
def build_ticket(header: 'OrderHeader', details: list['OrderItem'], table: 'TableSeat', width: int = 42, title: str = "KITCHEN") -> str:
    ...
    # --- 詳細デバッグログの追加 ---
    current_app.logger.info(f"DEBUG: build_ticketに渡されたtableオブジェクト: {table}")
    ...
    current_app.logger.info(f"DEBUG: tableオブジェクトから取得されたtable_no: {table_no}")
    
    if not table_no:
        table_id_for_debug = getattr(header, 'table_id', 'N/A')
        current_app.logger.warning(f"WARNING: table_noが不正なため、フォールバックを使用します。header.table_id: {table_id_for_debug}")
...
def build_ticket_with_totals(header, items, table, new_item_ids):
    ...
    left_margin = "   "  # 左端に半角スペース3つの余白
    pad = lambda s: left_margin + (s[:width]).ljust(width)
    hr = left_margin + "-" * 45  # 点線の長さを調整（左側余白3文字分を減らす）
    ...
    current_app.logger.debug(f"[build_ticket] filtered new_items count={len(new_items)}")
    ...
    if order_time is not None:
        try:
            ...
            current_app.logger.error(f"[build_ticket] datetime conversion error: {e}")
            order_time_str = str(order_time)
    else:
        order_time_str = 'N/A'
    ...
    # 商品名(22文字) + 数量(4文字右寄せ) + スペース + 金額(16文字右寄せ) = 43文字
    # 手動調整: 全角文字の幅を考慮して固定文字列を使用
    lines.append(pad("商品名                数量             金額"))
    lines.append(hr)
    
    # 今回送信した商品のみを表示（OrderItem IDでフィルタリング）
    current_app.logger.debug(f"[build_ticket] new_item_ids={new_item_ids}")
    current_app.logger.debug(f"[build_ticket] all items count={len(items)}")
    for item in items:
        current_app.logger.debug(f"[build_ticket] item.id={getattr(item, 'id', None)}, menu_id={getattr(item, 'menu_id', None)}")
    # new_itemsは既に上で定義済み
...
# --- ヘルパー関数：カテゴリ階層チェック --------------------------------------------
def _is_descendant_category(session_db, child_id, parent_id):
    """
    カテゴリが別のカテゴリの子孫かどうかをチェック
    ...
    Returns:
        bool: child_idがparent_idの子孫である場合True
    """
    # 直接の親をチェック
    current_id = child_id
    visited = set()  # 循環参照対策
    ...
    while current_id and current_id not in visited:
        ...
        category = session_db.get(Category, current_id)
        if not category:
            return False
...
def _trigger_kds_print(session_db, order_id, new_items):
    ...
    # 印刷ルールを取得
    rules = session_db.query(PrintRule).all()
    if not rules:
        current_app.logger.debug("[_trigger_kds_print] no print rules found")
        return
    ...
    for item in new_items:
        ...
        for rule in rules:
            ...
            # カテゴリ指定のルール
                ...
                # 直接マッチしない場合、子カテゴリもチェック（階層対応）
                # メニューが属するカテゴリを取得
                menu_categories = session_db.query(ProductCategoryLink.category_id).filter(
                    ProductCategoryLink.product_id == menu_id
                ).all()
                menu_category_ids = [c[0] for c in menu_categories]
                
                # 各カテゴリが指定されたカテゴリの子孫かチェック
                for cat_id in menu_category_ids:
                    if _is_descendant_category(session_db, cat_id, rule.category_id):
                        if rule.printer_id not in printer_items:
                            printer_items[rule.printer_id] = []
                        printer_items[rule.printer_id].append(item)
                        break  # 最初にマッチしたルールを適用
    ...
    if not printer_items:
        current_app.logger.debug("[_trigger_kds_print] no items matched print rules")
        return
    
    # 注文情報を取得
    order = session_db.get(OrderHeader, order_id)
    if not order:
        current_app.logger.warning("[_trigger_kds_print] order not found: %s", order_id)
        return
    ...
    for printer_id, items in printer_items.items():
        ...
        if not printer or printer.enabled != 1:
            current_app.logger.warning("[_trigger_kds_print] printer not found or disabled: %s", printer_id)
            continue
        ...
        try:
            ...
            if printer.kind == "escpos_tcp":
                ...
                current_app.logger.info("[_trigger_kds_print] printed to %s (%s): %d items",
                               printer.name, printer.connection, len(items))
            elif printer.kind == "printer_server":
                # printer_serverタイプの場合、Printer Server EXEが自動的にOrderHeaderをポーリングして印刷する
                # ここでは何もする必要がない
                current_app.logger.info("[_trigger_kds_print] printer_server type: %s will be handled by Printer Server EXE (order_id=%s)",
                               printer.name, order_id)
            else:
                current_app.logger.warning("[_trigger_kds_print] unsupported printer type: %s", printer.kind)
        except Exception as e:
            current_app.logger.exception("[_trigger_kds_print] failed to print to %s: %s", printer.name, e)
...
def trigger_print_job(order_id: int, items_to_print: list = None):
    ...
    try:
        ...
        if not header:
            current_app.logger.warning(f"注文ヘッダーが見つかりませんでした: {order_id}")
            return
        ...
        # --- 詳細なデバッグログを追加 ---
        current_app.logger.info(f"DEBUG: 注文ID {order_id} に紐づくテーブルID: {getattr(header, 'table_id', 'N/A')}")
        if table:
            current_app.logger.info(f"DEBUG: データベースからTableSeatオブジェクトを正常に取得しました。")
            current_app.logger.info(f"DEBUG: 取得されたテーブルID: {getattr(table, 'id', 'N/A')}")
            current_app.logger.info(f"DEBUG: 取得されたテーブル番号: {getattr(table, 'table_no', 'N/A')}")
            if not getattr(table, 'table_no', None):
                current_app.logger.warning("WARNING: Table object found, but 'table_no' attribute is missing or empty.")
        else:
            current_app.logger.warning(f"テーブルが見つかりませんでした。header.table_id: {getattr(header, 'table_id', 'N/A')}")
            
        current_app.logger.info(f"注文ID {order_id} の印刷ジョブを開始します。")
        ...
        current_app.logger.info(f"注文ID {order_id} に紐づく明細が {len(details)} 件見つかりました。")
        ...
        for item in details:
            current_app.logger.info(f"明細ID {item.id} (メニューID: {item.menu_id}) のプリンタを解決します。")
            printers = resolve_printers_for_item(s, item.menu_id)
            current_app.logger.info(f"明細ID {item.id} には {len(printers) if printers else 0} 件のプリンタが割り当てられました。")
        ...
        current_app.logger.info(f"合計 {len(printer_buckets)} 件のプリンタバケットが作成されました。")
        ...
        for key, vals in printer_buckets.items():
            ...
            if items_to_print_in_bucket:
                ...
                current_app.logger.info(f"プリンタID {key if key else 'DEFAULT'} に {len(items_to_print_in_bucket)} 件の明細を印刷します。")
                ...
                current_app.logger.warning(f"プリンタID {key if key else 'DEFAULT'} に印刷する明細がありません。")
...
def trigger_print_async(order_id: int) -> None:
    ...
    def _run(oid: int) -> None:
        try:
            ...
            current_app.logger.error("[print] async failed: %s", e, exc_info=True)
    ...
    if store_id is not None:
        ...
        if sid is not None:
            ...
            try:
                ...
                current_app.logger.warning(f"[login_session] 店舗IDマスター登録(コード)に失敗: {e}")

            # 2) 主キー=店舗ID==sid の行を必ず用意（無ければINSERT）
            try:
                _ensure_master_pk_equals_store_id(sid, store_name)
            except Exception as e:
                current_app.logger.error(f"[login_session] 店舗IDマスター(主キー整合)に失敗: {e}")

            # 3) 最終確認ログ
            try:
                ok = validate_store_id(sid)
                current_app.logger.info(f"[login_session] validate_store_id({sid})={ok}")
            except Exception as e:
                current_app.logger.warning(f"[login_session] validate_store_id 例外: {e}")
...
def _ensure_master_pk_equals_store_id(sid: int, store_name: str | None):
    ...
    try:
        ...
        if row is None:
            ...
            s.commit()
            current_app.logger.info(f"[_ensure_master_pk_equals_store_id] inserted sid={sid}")
        else:
            s.execute(text('''
                UPDATE "M_店舗IDマスター"
                   SET "有効フラグ"=1,
...
@bp.route("/admin/price-mode", methods=["POST"])
def set_price_mode():
    """[価格入力モード設定] cookie に incl/excl を1年保持してリダイレクト"""
    mode = (request.form.get("mode") or "").lower()
    if mode not in ("incl", "excl"):
        abort(400)
    resp = make_response(redirect(request.referrer or url_for("pos_app.floor")))
...
# --- システム管理者ログイン -----------------------------------------------------
@bp.route("/login/sysadmin", methods=["GET", "POST"])
def sysadmin_login():
    s = SessionLocal()
    try:
        if request.method == "POST":
            ...
            if not (user and check_password_hash(user.password_hash, password)):
                return render_template("pos/sysadmin_login.html", error="ログインIDまたはパスワードが違います。")
            user.last_login = now_str(); s.commit()
            login_session("sysadmin", user.id, user.name, None, None)
            # ↓ ここを floor → dev_tools に変更
            return redirect(request.args.get("next") or url_for("pos_app.dev_tools"))
        return render_template("pos/sysadmin_login.html")
    finally:
        s.close()
        SessionLocal.remove()
...
# --- システム管理者 初回ブートストラップ ----------------------------------------
@bp.route("/sysadmin/bootstrap", methods=["GET", "POST"])
def sysadmin_bootstrap():
    s = SessionLocal()
    try:
        exists = s.query(SysAdmin.id).first()
        if exists:
            return redirect(url_for("pos_app.sysadmin_login"))
        if request.method == "POST":
            login_id = (request.form.get("login_id") or "").strip()
            password = (request.form.get("password") or "")
            ...
            if not (login_id and password and password_confirm and name):
                return render_template("pos/sysadmin_bootstrap.html", error="全ての項目を入力してください。")

            if password != password_confirm:
                return render_template("pos/sysadmin_bootstrap.html", error="パスワードが一致しません。")
            ...
            s.commit()
            return redirect(url_for("pos_app.sysadmin_login"))
        return render_template("pos/sysadmin_bootstrap.html")
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/login/tenant", methods=["GET", "POST"])
def tenant_admin_login_entry():
    if request.method == "POST":
        slug = (request.form.get("tenant_slug") or "").strip()
        if not slug:
            return render_template("pos/tenant_admin_login_select.html",
                                   error="テナントを入力してください。")
        return redirect(url_for("pos_app.tenant_admin_login", tenant_slug=slug))
    return render_template("pos/tenant_admin_login_select.html")


# --- テナント管理者ログイン（正式：/t/<slug>/login/tenant） ---------------------
@bp.route("/t/<tenant_slug>/login/tenant", methods=["GET", "POST"])
def tenant_admin_login():
    rid = uuid.uuid4().hex[:8]
    current_app.logger.info(f"[tenant_login {rid}] {request.method} from {request.remote_addr} ua={request.user_agent}")
    ...
    try:
        if request.method == "POST":
            ...
            if not user:
                return render_template("pos/tenant_admin_login.html", error="アカウントが見つかりません。")
            if not user.active:
                return render_template("pos/tenant_admin_login.html", error="このアカウントは無効です。")
            if not user.password_hash:
                return render_template("pos/tenant_admin_login.html", error="パスワード未設定です（管理者に連絡してください）。")
            try:
                ok = check_password_hash(user.password_hash, password)
            except Exception:
                current_app.logger.exception(f"[tenant_login {rid}] check_password_hash failed")
                ok = False
            if not ok:
                return render_template("pos/tenant_admin_login.html", error="パスワードが違います。")
            ...
            return redirect(request.form.get("next") or request.args.get("next") or url_for("pos_app.tenant_portal"))

        return render_template("pos/tenant_admin_login.html")
    finally:
        s.close()  # コメント必須
        SessionLocal.remove()
...
@bp.route("/sysadmin/tenants/<int:tid>/admins/update", methods=["POST"])
@require_sysadmin
    ...
    try:
        ...
        if op in ("toggle", "resetpw", "delete"):
            ...
            if not u or int(u.tenant_id or 0) != int(tid):
                ...
                return redirect(url_for("pos_app.sys_tenant_admins", tid=tid))
...
# --- テナント管理：店舗編集 ------------------------------------------------------
@bp.route("/tenant/stores/<int:sid>/edit", methods=["GET","POST"])
...
def tenant_store_edit(sid):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            s.commit()
            flash("更新しました。")
            return redirect(url_for("pos_app.tenant_stores"))
        # GET
        return render_template("pos/tenant_store_edit.html", store=st)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- テナント管理：店舗削除 ------------------------------------------------------
@bp.route("/tenant/stores/<int:sid>/delete", methods=["POST"])
@require_tenant_admin
...
def admin_login():
    ...
    try:
        if request.method == "POST":
            ...
            if not store_result:
                return render_template("pos/admin_login.html", error="店舗コードが正しくありません。")
            ...
            if not (user and check_password_hash(user.password_hash, password)):
                return render_template("pos/admin_login.html", 
            ...
            return redirect(request.args.get("next") or url_for("pos_app.floor"))
...
# --- 従業員ログイン --------------------------------------------------------------
@bp.route("/login/staff", methods=["GET", "POST"])
def staff_login():
    s = SessionLocal()
    try:
        if request.method == "POST":
            ...
            if not store_row:
                return render_template("pos/staff_login.html", error="店舗コードが正しくありません。")
            ...
            return redirect(request.args.get("next") or url_for("pos_app.staff_floor"))

        return render_template("pos/staff_login.html")
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 初回セットアップ（店舗が無い時だけ許可） -----------------------------------
@bp.route("/admin/bootstrap", methods=["GET", "POST"])
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not (st_code and st_name and ad_id and ad_pw and ad_name):
                return render_template("pos/bootstrap.html", error="全ての項目を入力してください。")
            if s.query(Store).filter(Store.code == st_code).first():
                return render_template("pos/bootstrap.html", error="その店舗コードは既に使われています。")
            ...
            return redirect(url_for("pos_app.admin_login"))
        return render_template("pos/bootstrap.html")
    finally:
        s.close()
        SessionLocal.remove()
...
def dev_tools_redirect():
    ...
    return redirect(url_for("pos_app.dev_tools"))


# --- スキーマ軽量マイグレーション実行（ensure + tenant列追加） -------------------
@bp.route("/sys/migrate", methods=["POST"])
@require_sysadmin
def sys_migrate():
    # 既存ユーティリティの実行
...
# --- テナント一覧表示 ------------------------------------------------------------
@bp.route("/sysadmin/tenants")
@require_sysadmin
def sys_tenants():
    s = SessionLocal()
    try:
        rows = s.query(M_テナント).order_by(M_テナント.id.desc()).all()
        return render_template("pos/sys_tenants.html", rows=rows)
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/sysadmin/tenants/new", methods=["GET", "POST"])
@require_sysadmin
def sys_tenants_new():
    if request.method == "GET":
        # 入力フォームを表示
        return render_template("pos/sys_tenant_new.html")
    ...
    try:
        if not name or not slug:
            flash("名称とslugは必須です")
            return render_template("pos/sys_tenant_new.html", name=name, slug=slug)
        ...
        s.commit()
        flash("テナントを作成しました")
        return redirect(url_for("pos_app.sys_tenants"))
    finally:
        s.close()
        SessionLocal.remove()
...
# --- テナント管理者の新規作成 ----------------------------------------------------
@bp.route("/sysadmin/tenants/<int:tid>/admins/new", methods=["GET", "POST"])
@require_sysadmin
def sys_tenant_admins_new(tid):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            return redirect(url_for("pos_app.sys_tenant_admins", tid=t.id))

        # GET: フォーム
        return render_template("pos/sys_tenant_admin_new.html", tenant=t)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- テナント管理者：自分のパスワード変更 ---------------------------------------
@bp.route("/tenant/me/resetpw", methods=["POST"])
@require_tenant_admin
def tenant_me_resetpw():
    ...
    try:
        ...
        if not (uid and pw):
            flash("パスワードを入力してください。")
            return redirect(url_for("pos_app.tenant_portal"))
        u = s.get(TenantAdmin, uid)
        if not u:
            flash("ユーザーが見つかりません。")
            return redirect(url_for("pos_app.tenant_portal"))
...
@bp.route("/tenant/me/edit", methods=["GET", "POST"])
@require_tenant_admin
def tenant_me_edit():
    s = SessionLocal()
    try:
        ...
        # GET
        return render_template("pos/tenant_me_edit.html", t=t, back_url=back_url)
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/tenant/me/delete", methods=["POST"])
@require_tenant_admin
...
# --- テナント管理者一覧（指定テナント） -----------------------------------------
@bp.route("/sysadmin/tenants/<int:tid>/admins")
@require_sysadmin
def sys_tenant_admins(tid):
    ...
    try:
        ...
        return render_template("pos/sys_tenant_admins.html", tenant=t, admins=admins)
    finally:
        s.close()
        SessionLocal.remove()
...
def tenant_portal():
    ...
    try:
        ...
        return render_template("pos/tenant_portal.html",
                               tenant=tenant,
                               admin=ta,
...
# --- 店舗一覧（テナント内） ------------------------------------------------------
@bp.route("/tenant/stores")
@require_tenant_admin
def tenant_stores():
    s = SessionLocal()
    try:
        rows = s.query(Store).order_by(Store.id.desc()).all()
        return render_template("pos/tenant_stores.html", rows=rows)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 店舗新規作成（POST） --------------------------------------------------------
@bp.route("/tenant/stores/new", methods=["POST"])
@require_tenant_admin
...
# --- 店舗管理者の新規作成（指定店舗） -------------------------------------------
@bp.route("/tenant/stores/<int:sid>/admins/new", methods=["GET", "POST"])
@require_tenant_admin
def tenant_store_admin_new(sid):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            flash("店舗管理者を作成しました")
            return redirect(url_for("pos_app.tenant_stores"))
        return render_template("pos/tenant_store_admin_new.html", store=st)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- テナント編集（名称・slug 更新） ---------------------------------------------
@bp.route("/sysadmin/tenants/<int:tid>/edit", methods=["GET", "POST"])
@require_sysadmin
def sys_tenant_edit(tid):
    ...
    try:
        ...
        return render_template("pos/sys_tenant_edit.html", t=t)
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/sysadmin/tenants/<int:tid>/delete", methods=["POST"])
@require_sysadmin
...
@bp.route("/sysadmin/tenants/<int:tid>/purge", methods=["POST"])
@require_sysadmin
...
# --- テナント管理者：店舗管理者の削除 -------------------------------------------
@bp.route("/tenant/stores/<int:sid>/admins/<int:aid>/delete", methods=["POST"])
@require_tenant_admin
...
def sys_mypage():
    ...
    try:
        ...
        return render_template("pos/sys_mypage.html", me=me)
    finally:
        s.close()
        SessionLocal.remove()
...
def sys_admins():
    ...
    try:
        ...
        return render_template("pos/sys_admins.html", users=users)
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/sys/admins/update", methods=["POST"])
@require_sysadmin
def sys_admins_update():
    ...
    try:
        if op == "add":
            ...
            password = (f.get("password") or "")
            if not (login_id and name and password):
                flash("必須項目が不足しています。"); return redirect(url_for("pos_app.sys_admins"))
            if s.query(SysAdmin.id).filter(SysAdmin.login_id == login_id).first():
                flash("同じログインIDが既に存在します。"); return redirect(url_for("pos_app.sys_admins"))
            s.add(SysAdmin(login_id=login_id,
                           password_hash=generate_password_hash(password),
                           name=name, active=1,
                           created_at=now_str(), updated_at=now_str()))
        ...
        return redirect(url_for("pos_app.sys_admins"))
    finally:
        s.close()
        SessionLocal.remove()
...
def _coalesce_name_expr(sess, table: str,
    ...
    try:
        ...
        try:
            sess.rollback()
            current_app.logger.warning(f"[_coalesce_name_expr] カラム取得失敗 table={table}: {e}")
            # ロールバック後、新しいトランザクションを開始
            sess.begin()
        except Exception as rollback_error:
            current_app.logger.error(f"[_coalesce_name_expr] ロールバック失敗: {rollback_error}")
        cols = set()
...
def _coalesce_name_expr_alias(sess, table: str, table_alias="s",
    ...
    try:
        ...
        try:
            sess.rollback()
            current_app.logger.warning(f"[_coalesce_name_expr_alias] カラム取得失敗 table={table}: {e}")
            # ロールバック後、新しいトランザクションを開始
            sess.begin()
        except Exception as rollback_error:
            current_app.logger.error(f"[_coalesce_name_expr_alias] ロールバック失敗: {rollback_error}")
        cols = set()
...
def _recalc_order_totals_with_negatives_db(s, order_id: int, order_obj=None):
    ...
    if debug_on:
        ...
        try:
            ...
            current_current_app.logger.debug("[recalc.items] order_id=%s count=%s items=%s",
    ...
    if debug_on:
        try:
            ...
            current_current_app.logger.debug("[recalc.before] order_id=%s %s", order_id, before)
        except Exception:
            pass
    ...
    if debug_on:
        try:
            ...
            current_current_app.logger.debug("[recalc.after] order_id=%s %s", order_id, after)
        except Exception:
            pass
...
@bp.route("/admin/mypage", methods=["GET", "POST"])
@require_admin
def admin_mypage():
    s = SessionLocal()
    try:
        ...
        if not admin_id:
            ...
            return redirect(url_for("pos_app.admin_login", next=_choose_next(url_for("pos_app.index"))))
        ...
        if request.method == "POST":
            ...
            if action == "profile":
                ...
                s.commit()
                flash("プロフィールを更新しました。")
                return redirect(url_for("pos_app.admin_mypage"))
...
@bp.route("/switch_store/<int:store_id>")
def switch_store(store_id: int):
    role = session.get("role")
    default_next = (
        url_for("pos_app.staff_mypage") if role == "staff"
        else url_for("pos_app.admin_mypage") if role in ("store_admin", "tenant_admin", "sysadmin")
        else url_for("pos_app.index")
    )
    ...
    if not session.get("logged_in"):
        if role == "staff":
            return redirect(url_for("pos_app.staff_login", next=next_url))
        elif role in ("store_admin", "tenant_admin", "sysadmin"):
            return redirect(url_for("pos_app.admin_login", next=next_url))
        else:
            return redirect(url_for("pos_app.login_choice"))
...
@bp.route("/staff/mypage", methods=["GET", "POST"])
@require_staff
def staff_mypage():
    s = SessionLocal()
    try:
        ...
        if not staff_id:
            ...
            return redirect(url_for("pos_app.staff_login", next=_choose_next(url_for("pos_app.index"))))
        ...
        if request.method == "POST":
            ...
            if action == "profile":
                ...
                s.commit()
                flash("プロフィールを更新しました。")
                return redirect(url_for("pos_app.staff_mypage"))
...
# 既存データの修復スクリプト
# ---------------------------------------------------------------------
@bp.route("/admin/fix_progress_data", methods=["POST"])
@require_admin
...
def _guess_tax_rate(src_item=None, menu=None):
    ...
    if cand is None and src_item is not None:
        ...
        try:
            ...
            cand = None
    if cand is None:
        cand = current_current_app.config.get("TAX_RATE_DEFAULT", 0.10)
    try:
        return float(cand)
    except Exception:
        return 0.10
...
def staff_api_order_item_status(item_id: int):
    ...
    try:
        ...
        s.rollback()
        current_current_app.logger.exception("order_item status update failed")
...
@bp.route("/admin/console", endpoint="admin_console")
...
# 店舗情報編集ページ（レシート・領収書用）
# ---------------------------------------------------------------------
@bp.route("/admin/store-info", methods=["GET", "POST"], endpoint="admin_store_info")
@require_admin
def admin_store_info():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            return redirect(url_for("pos_app.admin_store_info"))
...
@bp.route("/admin/console/join-pin", methods=["POST"], endpoint="admin_toggle_join_pin")
@require_admin
def admin_toggle_join_pin():
    ...
    try:
        ...
        s.rollback()
        current_current_app.logger.exception("failed to toggle join pin")
...
def admin_table_sales():
    ...
    try:
        ...
        # ★★★ デバッグ：取得した伝票を出力 ★★★
        current_current_app.logger.info("[ADMIN SALES DEBUG] 取得した伝票数: %d", len(rows))
        for h in rows[:5]:  # 最初の5件だけ
            current_current_app.logger.info("[ADMIN SALES DEBUG] 伝票ID=%s status=%s", 
                                   getattr(h, "id", None), 
        ...
        # ★★★ デバッグ：order_ids を出力 ★★★
        current_current_app.logger.info("[ADMIN SALES DEBUG] order_ids: %s", order_ids[:10])
        ...
        if order_ids:
            ...
            current_current_app.logger.info("[ADMIN SALES DEBUG] OrderItem columns: %s", 
            ...
            try:
                ...
                current_current_app.logger.info("[ADMIN SALES DEBUG] 生SQL明細数: %d", raw_count)
                ...
                # ★★★ デバッグ：取得した明細数を出力 ★★★
                current_current_app.logger.info("[ADMIN SALES DEBUG] 取得した明細数: %d", len(items))
                for it in items[:5]:  # 最初の5件だけ
                    current_current_app.logger.info("[ADMIN SALES DEBUG] 明細ID=%s order_id=%s qty=%s", 
                                           getattr(it, "id", None),
                ...
                for it in items:
                    try:
                        ...
                        current_current_app.logger.debug(
        ...
        current_current_app.logger.info("[ADMIN SALES DEBUG] items_map keys: %s", list(items_map.keys())[:10])
        for oid, items in list(items_map.items())[:3]:
            current_current_app.logger.info("[ADMIN SALES DEBUG] order_id=%s 明細数=%d", oid, len(items))
...
# --- 返金（伝票単位） ------------------------------
@bp.route("/admin/orders/<int:order_id>/refund", methods=["POST"])
@require_store_admin
def admin_order_refund(order_id):
    ...
    - デバッグ: current_app.config['DEBUG_REFUND']=True で詳細ログ
    """
    # ---- debug helpers (fallback; グローバル未定義でも動く) ----
    def __dbg_enabled(key: str) -> bool:
        try:
            from flask import current_app
            return bool(current_current_app.config.get(key, False))
        except Exception:
            return False
    ...
    def __dbg(ctx: str, **kv):
        ...
        try:
            import json
            from flask import current_app
            current_current_app.logger.debug(f"[{ctx}] " + json.dumps(kv, ensure_ascii=False, default=str))
        except Exception:
            pass
    ...
    try:
        ...
        s.rollback()
        current_current_app.logger.exception("[admin_order_refund] %s", e)
        if __dbg_enabled('DEBUG_REFUND'):
...
# --- 明細の取消（部分数量対応）＋自動返金 ---------------------------------------------
@bp.route("/admin/orders/<int:order_id>/items/<int:item_id>/cancel", methods=["POST"])
@require_store_admin
def admin_order_item_cancel(order_id, item_id):
    ...
    - 進捗カウンタ（T_明細進捗）を更新
    - デバッグ: current_app.config['DEBUG_CANCEL']=True / current_app.config['DEBUG_TOTALS']=True
    """
    ...
    def __dbg(ctx: str, **kv):
        ...
        try:
            ...
            try:
                current_current_app.logger.debug(f"[{ctx}] {kv!r}")
            except Exception:
                pass
    ...
    try:
        ...
        try:
            ...
            current_current_app.logger.warning("[cancel.progress] failed to update progress: %s", e)
            # 進捗更新失敗は致命的ではないので続行
        ...
        try:
            s.flush()  # ID採番・関連の確定
            if bool(current_current_app.config.get("DEBUG_TOTALS", False)):
                hdr = it.order
                current_current_app.logger.debug(
            ...
            current_current_app.logger.exception("[cancel.recalc] failed but continue")
        # ▲▲ ここまで ▲▲
        ...
        s.rollback()
        current_app.logger.exception("[admin_order_item_cancel] %s", e)
        if current_current_app.config.get("DEBUG_CANCEL"):
            return jsonify({"ok": False, "error": "internal error", "detail": str(e)}), 500
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
...
def floor():
    ...
    def dbg(msg):
        try:
            current_app.logger.info(f"[FLOOR-DEBUG] {msg}")
        except Exception:
            pass
...
@bp.route("/qr/print/<int:table_id>")
@require_any
def qr_print(table_id: int):
    ...
    try:
        ...
        menu_url = url_for("pos_app.menu_page", tenant_slug=tenant_slug, token=token, _external=True)
        ...
        try:
            ...
            current_app.logger.exception("[QR] build failed")
...
@bp.route("/floor/table/<int:table_id>")
def table_detail(table_id):
    ...
    try:
        ...
        def _item_to_dict(it):
            ...
            try:
                ...
                try:
                    ...
                    pass
                current_app.logger.warning(f"[_item_to_dict] menu access failed: {e}")
            ...
            if item_id and abs(qty) >= 1:
                try:
                    ...
                    current_app.logger.debug(f"[_item_to_dict] item_id={item_id} progress={progress}")
                except Exception as e:
                    current_app.logger.warning(f"[_item_to_dict] progress_get failed for item_id={item_id}: {e}")
                    try:
                        s.rollback()
                    except:
                        pass
        ...
        if bool(current_current_app.config.get("DEBUG_TOTALS", False)) and headers:
            rid = headers[0].id
            current_app.logger.debug("[table_detail.totals] order_id=%s total=%s items=%s",
        ...
        return render_template("pos/table_detail.html", **context)
    finally:
        s.close()
        SessionLocal.remove()
...
def kds():
    ...
    try:
        ...
        return render_template("pos/kds.html", title="KDS", rows=rows_dict, use_cooking_status=use_cooking_status)
    finally:
        s.close()
        SessionLocal.remove()
...
def kds_api_get_items():
    ...
    try:
        ...
        if DEBUG:
            current_current_app.logger.debug("[KDS] sid=%s raw_cat_ids=%r -> cat_ids=%r", sid, raw, cat_ids)
        ...
        if cat_ids:
            ...
            """), {"sid": sid}).scalars().all()
            if DEBUG:
                current_current_app.logger.debug("[KDS] fetched menu_ids len=%d sample=%r",
        ...
        if DEBUG:
            current_current_app.logger.debug("[KDS] base rows=%d ids(sample)=%r",
        ...
        def fetch_progress_for_ids(id_list):
            ...
            try:
                ...
                current_app.logger.warning(f"[fetch_progress_for_ids] T_明細進捗テーブルへのアクセスに失敗: {e}")
                s.rollback()
            return out
        ...
        def ensure_progress_has_status():
            try:
                ...
                if "status" not in cols:
                    ...
                    s.commit()
                    if DEBUG:
                        current_current_app.logger.debug("[KDS] progress table: added status column with DEFAULT '新規'")
            except Exception as e:
                try:
                    s.rollback()
                except Exception:
                    pass
                current_current_app.logger.warning("[KDS] ensure_progress_has_status failed: %s", e)
        ...
        if seeded and DEBUG:
            current_current_app.logger.debug("[KDS] progress seeded item_ids=%r", seeded)
        ...
        if DEBUG:
            ...
            current_current_app.logger.debug("[KDS] debug=%r", dbg)
            return jsonify(ok=True, items=items, debug=dbg)

        return jsonify(ok=True, items=items)

    except Exception as e:
        current_current_app.logger.exception("KDS API error: %s", e)
        return jsonify(ok=False, error="internal error"), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@require_admin
def admin_kds_home():
    return render_template("pos/admin_kds_home.html", title="KDS管理")


# --- KDS カテゴリ管理（作成/更新/削除） ---------------------------------------
@bp.route("/admin/kds/categories", methods=["GET", "POST"])
@require_admin
def admin_kds_categories():
    ...
    try:
        if request.method == "POST":
            ...
            if act == "create":
                ...
                if name:
                    ...
                    s.commit()
                return redirect(url_for('pos_app.admin_kds_categories'))
        ...
        return render_template("pos/admin_kds_categories.html", cats=cats, title="KDSカテゴリ")
...
@bp.route("/admin/kds/mapping", methods=["GET", "POST"])
@require_admin
def admin_kds_mapping():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            return redirect(url_for("pos_app.admin_kds_mapping"))
...
@bp.route("/assets/order_notify.wav")
...
def api_menus_by_category(category_id: int):
    ...
    try:
        ...
        current_app.logger.error("[api_menus_by_category] %s", e, exc_info=True)
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def api_menus_by_categories():
    ...
    try:
        ...
        current_app.logger.error("[api_menus_by_categories] %s", e, exc_info=True)
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/admin/order/<int:order_id>/detail/json")
@require_any  # ← staff/admin どちらでも可。必要に応じて @require_staff 等に変更
def order_detail_json(order_id: int):
    sid = current_store_id()
    current_app.logger.info(f"[DETAIL-JSON] in sid={sid} order_id={order_id}")

    if sid is None:
        current_app.logger.warning("[DETAIL-JSON] no store in session")
    ...
    try:
        h = s.get(OrderHeader, order_id)
        if not h:
            current_app.logger.warning(f"[DETAIL-JSON] order not found: {order_id}")
            return jsonify(ok=False, error="order not found"), 404
        if hasattr(h, "store_id") and h.store_id != sid:
            current_app.logger.warning(f"[DETAIL-JSON] store mismatch: sid={sid} header.sid={getattr(h,'store_id',None)}")
        ...
        current_app.logger.info(f"[DETAIL-JSON] items={len(items)}")
        ...
        current_app.logger.info(f"[DETAIL-JSON] send totals excl={payload['order']['total_excl']} incl={payload['order']['total_incl']}")
        return jsonify(payload)

    except Exception as e:
        current_app.logger.exception("[DETAIL-JSON] unexpected error")
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# --- [Order API] 会計完了（残額=0 確認→クローズ） -----------------------------------------------
@bp.route("/admin/order/<int:order_id>/complete", methods=["POST"])
@require_staff
def order_complete(order_id: int):
    ...
    try:
        ...
        try:
            ...
            current_current_app.logger.exception("reset customer detail after checkout failed")
        ...
        try:
            ...
            current_current_app.logger.exception("route-level fallback delete failed")
        ...
        s.rollback()
        current_current_app.logger.exception("order_complete failed")
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def __probe_customer_detail():
    ...
    try:
        ...
        current_current_app.logger.exception("__probe_customer_detail failed")
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/admin/order/<int:order_id>/void-payments", methods=["POST"])
@require_staff
...
def public_order_detail_json(order_id: int):
    ...
    try:
        ...
        current_app.logger.exception("[PUBLIC-DETAIL-JSON] unexpected error")
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 会計サマリ（取消除外版） ------------------------------------------
@bp.route("/admin/order/<int:order_id>/summary")
@require_store_admin
def admin_order_summary(order_id: int):
    ...
    try:
        ...
        for d in items:
            ...
            if menu:
                current_app.logger.debug(f"[summary] item_id={getattr(d, 'id', None)} menu.id={menu.id} menu.name={getattr(menu, '名称', None)} menu.is_market_price={getattr(menu, 'is_market_price', None)} menu.時価={getattr(menu, '時価', None)} is_market_price={is_market_price} actual_price={actual_price}")
            else:
                current_app.logger.debug(f"[summary] item_id={getattr(d, 'id', None)} menu=None")
        ...
        s.rollback()
        current_app.logger.exception("[admin_order_summary] %s", e)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 時価商品の価格更新 -----------------------------
@bp.route("/admin/order_item/<int:item_id>/set_price", methods=["POST"])
@require_store_admin
def set_market_price(item_id):
    """
    時価商品の実際の価格を設定する
    """
    current_app.logger.info("[set_market_price] START item_id=%s", item_id)
    data = request.get_json(force=True) or {}
    current_app.logger.info("[set_market_price] data=%s", data)
    price = data.get("price")
    price_mode = data.get("mode", data.get("price_mode", "excl"))  # "excl" or "incl"
    current_app.logger.info("[set_market_price] price=%s, price_mode=%s", price, price_mode)
    ...
    try:
        ...
        current_app.logger.info("[set_market_price] tax_rate=%s", tax_rate)
        ...
        if price_mode == "incl":
            ...
            current_app.logger.info("[set_market_price] price_mode=incl: input_price=%s -> actual_price_excl=%s (rounded up)", price, actual_price_excl)
        else:
            # 税抜価格が入力された場合、そのまま使用
            actual_price_excl = int(price)
            current_app.logger.info("[set_market_price] price_mode=excl: actual_price_excl=%s", actual_price_excl)
        
        # 実際価格を設定（Pythonの属性名を使用）
        current_app.logger.info("[set_market_price] Setting actual_price=%s for item_id=%s", actual_price_excl, item_id)
        item.actual_price = actual_price_excl
        s.commit()
        current_app.logger.info("[set_market_price] Successfully committed price for item_id=%s, actual_price=%s", item_id, item.actual_price)
        
        return jsonify({"ok": True})
    except Exception as e:
        s.rollback()
        current_app.logger.exception("[set_market_price] %s", e)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 分割会計（取消除外の残額を基準に検証） -----------------------------
@bp.route("/admin/settle/<int:table_id>/pay", methods=["POST"])
@require_staff
def admin_settle_pay(table_id):
    ...
    try:
        ...
        s.rollback()
        current_app.logger.exception("[admin_settle_pay] %s", e)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/api/order_item/<int:item_id>/status", methods=["POST"])
@require_any
def api_order_item_status(item_id: int):
    """
    _progress_update_coreを呼び出す
    """
...
@bp.route("/kds/api/item/<int:item_id>/status", methods=["POST"])
@require_any
...
# --- 画面：メニュー管理トップ ---------------------------------------------------
@bp.route("/admin/menu/home", endpoint="admin_menu_home")
@require_admin
def admin_menu_home():
    return render_template("pos/menu_home.html", title="メニュー管理")


# --- 画面：メニュー新規作成フォーム --------------------------------------------
@bp.route("/admin/menu/new_form", endpoint="admin_menu_new_form")
@require_admin
...
@bp.route("/admin/menu/new", methods=["POST"], endpoint="admin_menu_new")
@require_admin
def admin_menu_new():
    f = request.form
    s = SessionLocal()
    try:
        ...
        if not ok:
            try:
                ...
                current_app.logger.error("[menu_new] ensure master error: %s", e, exc_info=True)
                return f"店舗ID {sid} の登録に失敗しました。", 500
        ...
        if file and file.filename:
            ...
            file.save(save_path)
            photo_url = url_for("pos_app.uploaded_file", filename=filename)
        ...
        s.commit()
        flash("メニューを登録しました。")
        return redirect(url_for("pos_app.admin_menu_list"))

    except Exception as e:
        s.rollback()
        current_app.logger.error("[admin_menu_new] insert failed: %s", e, exc_info=True)
...
# --- 画面：作成済みメニュー一覧 -------------------------------------------------
@bp.route("/admin/menu/list", endpoint="admin_menu_list")
@require_admin
def admin_menu_list():
    s = SessionLocal()
    try:
...
@bp.route("/api/admin/menus/by_category/<int:category_id>")
@require_admin
def api_admin_menus_by_category(category_id: int):
    ...
    try:
        ...
        current_app.logger.error("[api_admin_menus_by_category] %s", e, exc_info=True)
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def api_admin_category_bulk_available(cid: int):
    ...
    try:
        ...
        s.rollback()
        current_app.logger.error("[bulk_available] %s", e, exc_info=True)
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def api_admin_category_bulk_delete(cid: int):
    ...
    try:
        ...
        s.rollback()
        current_app.logger.error("[bulk_delete] %s", e, exc_info=True)
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/api/admin/menu/<int:mid>/toggle", methods=["POST"])
@require_admin
def api_admin_menu_toggle(mid: int):
    ...
    try:
        ...
        s.rollback()
        current_app.logger.error("[api_admin_menu_toggle] %s", e, exc_info=True)
        return jsonify(ok=False, error=str(e)), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def _dbg_push(debug_list, label, **data):
    ...
    try:
        current_app.logger.debug("[menu_edit][%s] %s", label, json.dumps(data, ensure_ascii=False, default=str))
    except Exception:
        current_app.logger.debug("[menu_edit][%s] %r", label, data)
...
@bp.route("/admin/menu/<int:mid>/edit", methods=["GET", "POST"], endpoint="admin_menu_edit")
@require_admin
def admin_menu_edit(mid):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not ok:
                try:
                    ...
                    current_app.logger.error(f"[menu_edit] ensure master error: {e}", exc_info=True)
            ...
            try:
                ...
                current_app.logger.error("[menu_edit] price/tax normalize failed: %s", e, exc_info=True)
            ...
            try:
                ...
                current_app.logger.error("[menu_edit] flush/force update failed: %s", e, exc_info=True)
                s.rollback()
            ...
            try:
                ...
                current_app.logger.error("[menu_edit] commit failed: %s", e, exc_info=True)
                s.rollback()
            ...
            if _is_debug_mode():
                ...
                body += f'<p><a href="{url_for("pos_app.admin_menu_list")}">メニュー一覧へ戻る</a></p>'
        ...
        current_app.logger.error("[menu_edit] unexpected error: %s", e, exc_info=True)
...
@bp.route("/admin/menu/<int:mid>/delete", methods=["POST"], endpoint="admin_menu_delete")
@require_admin
def admin_menu_delete(mid: int):
    ...
    try:
        ...
        try:
            ...
            s.rollback()
            current_app.logger.error("[menu_delete] logical delete failed: %s", e, exc_info=True)
        ...
        current_app.logger.error("[menu_delete] unexpected error: %s", e, exc_info=True)
        _dbg_push(debug_info, "unexpected_exception", error=str(e), traceback=traceback.format_exc())
        if _is_debug_mode():
...
def admin_menu_restore(mid: int):
    ...
    try:
        ...
        if not m:
            ...
            return redirect(url_for("pos_app.admin_menu_list_deleted"))
        ...
        s.rollback()
        current_current_app.logger.error("[menu_restore] failed: %s", e, exc_info=True)
...
def admin_menu_restore_and_enable(mid: int):
    ...
    try:
        ...
        s.rollback()
        current_current_app.logger.error("[menu_restore_and_enable] failed: %s", e, exc_info=True)
        flash("復元に失敗しました。", "error")
        return redirect(url_for("pos_app.admin_menu_list_deleted"))
    finally:
        s.close()
...
def admin_menu_list_deleted():
    ...
    try:
        ...
        return render_template("pos/menu_list_deleted.html", title="削除済みメニュー一覧", menus=menus)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 画面：支払方法一覧（管理者） ------------------------------------------------
@bp.route("/admin/payment_methods")
@require_admin
def admin_payment_methods():
    ...
    try:
        ...
        return render_template("pos/payment_methods.html", title="支払方法マスタ", rows=lst)
    except Exception as e:
...
# --- 登録：支払方法の新規追加 ----------------------------------------------------
@bp.route("/admin/payment_methods/new", methods=["POST"])
@require_admin
def admin_payment_methods_new():
    sid = current_store_id()
    if sid is None:
        flash("店舗が選択されていません。")
        return redirect(url_for("pos_app.admin_payment_methods"))
...
# --- 切替：支払方法の有効/無効トグル --------------------------------------------
@bp.route("/admin/payment_methods/<int:pid>/toggle", methods=["POST"])
@require_admin
...
@bp.route("/admin/payment_methods/<int:pid>/edit", methods=["POST"])
@require_admin
...
@bp.route("/admin/payment_methods/json")
@require_any
...
@bp.route("/admin/dev/rebuild_payment_method_unique")
@require_admin
...
@bp.route("/admin/dev/migrate_payment_method_store_id")
@require_admin
...
def admin_tables():
    ...
    try:
        ...
        return render_template("pos/tables.html", title="テーブル管理", tables=tables)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 登録：テーブル新規作成 -----------------------------------------------------
@bp.route("/admin/tables/new", methods=["POST"])
@require_admin
def admin_tables_new():
    ...
    try:
        ...
        s.add(rec); s.commit()
        return redirect(url_for("pos_app.admin_tables"))
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 画面：テーブルQR印刷ページ -------------------------------------------------
@bp.route("/admin/tables/<int:table_id>/qr")
@require_admin
def admin_table_qr(table_id):
    ...
    try:
        ...
        access_url = url_for("pos_app.menu_page", tenant_slug=slug, token=token, _external=True)

        # QR画像URL（下の png ルート）
        png_url    = url_for("pos_app.qr_png", tenant_slug=slug, token=token)
...
# --- 画像：テーブルQRのPNGを返す ------------------------------------------------
@bp.route("/t/<tenant_slug>/qr/<token>.png")
def qr_png(tenant_slug, token):
    url = url_for("pos_app.menu_page", tenant_slug=tenant_slug, token=token, _external=True)
    img = qrcode.make(url)
    buf = io.BytesIO()
    img.save(buf, format="PNG")
    buf.seek(0)
...
def admin_categories():
    ...
    try:
        ...
        return render_template("pos/categories.html", title="カテゴリ管理", cats=out)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 登録：カテゴリ新規作成 -----------------------------------------------------
@bp.route("/admin/categories/new", methods=["POST"])
@require_admin
def admin_categories_new():
    ...
    try:
        ...
        if sid is not None and hasattr(Category, "store_id"):
            rec.store_id = sid                         # ★ 店舗IDを保存
        s.add(rec); s.commit()
        return redirect(url_for("pos_app.admin_categories"))
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 画面/更新：カテゴリ編集 ----------------------------------------------------
@bp.route("/admin/categories/<int:cid>/edit", methods=["GET", "POST"])
@require_admin
def admin_categories_edit(cid):
    ...
    try:
        ...
        return render_template("pos/categories_edit.html", title="カテゴリ編集", cat=c, cat_options=cat_options)
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 切替：カテゴリの有効/無効トグル --------------------------------------------
@bp.route("/admin/categories/<int:cid>/toggle", methods=["POST"])
@require_admin
...
@bp.route("/admin/categories/<int:cid>/delete", methods=["POST"])
@require_admin
...
# --- 画面：商品オプション一覧 ---------------------------------------------------
@bp.route("/admin/product-options")
@require_admin
def admin_product_options():
    ...
    try:
        ...
        return render_template("pos/product_options_list.html", 
                             title="商品オプション管理", 
...
# --- 画面：商品オプション新規作成 -----------------------------------------------
@bp.route("/admin/product-options/new", methods=["GET", "POST"])
@require_admin
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not option_name:
                ...
                return redirect(url_for("pos_app.admin_product_options_new"))
            ...
            return redirect(url_for("pos_app.admin_product_options_edit", option_id=opt.id))
        ...
        return render_template("pos/product_options_new.html", 
                             title="商品オプション新規作成",
                             products=products)
    finally:
        s.close()
...
# --- 画面：商品オプション編集 ---------------------------------------------------
@bp.route("/admin/product-options/<int:option_id>/edit", methods=["GET", "POST"])
@require_admin
def admin_product_options_edit(option_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            return redirect(url_for("pos_app.admin_product_options_edit", option_id=option_id))
        ...
        return render_template("pos/product_options_edit.html", 
                             title="商品オプション編集",
                             option=opt,
                             choices=choices,
                             products=products,
...
# --- 削除：商品オプション削除 ---------------------------------------------------
@bp.route("/admin/product-options/<int:option_id>/delete", methods=["POST"])
@require_admin
def admin_product_options_delete(option_id):
    ...
    try:
        ...
        if used:
            ...
            return redirect(url_for("pos_app.admin_product_options"))
...
def admin_printers():
    s = SessionLocal()
    try:
        # トランザクションをリセットして最新データを取得
        s.expire_all()
        sid = current_store_id()
        tid = _current_tenant_id()
        logging.warning(f"[ADMIN-PRINTERS] Current store_id: {sid}")
        logging.warning(f"[ADMIN-PRINTERS] Current tenant_id: {tid}")
        logging.warning(f"[ADMIN-PRINTERS] hasattr(Printer, 'store_id'): {hasattr(Printer, 'store_id')}")
        q = s.query(Printer)
        if sid is not None and hasattr(Printer, "store_id"):
            logging.warning(f"[ADMIN-PRINTERS] Applying filter: Printer.store_id == {sid}")
            q = q.filter(Printer.store_id == sid)      # ★ 店舗で絞る
        else:
            logging.warning(f"[ADMIN-PRINTERS] No filter applied (sid={sid}, hasattr={hasattr(Printer, 'store_id')})")
        # SQLクエリをログ出力
        from sqlalchemy.dialects import postgresql
        compiled_query = q.statement.compile(dialect=postgresql.dialect(), compile_kwargs={"literal_binds": True})
        logging.warning(f"[ADMIN-PRINTERS] SQL Query (compiled): {compiled_query}")
        rows = q.order_by(Printer.id).all()
        logging.warning(f"[ADMIN-PRINTERS] Found {len(rows)} printers for store_id={sid}")
        # 生 SQL で確認
        from sqlalchemy import text
        raw_result = s.execute(text(f'SELECT COUNT(*) FROM "M_プリンタ" WHERE "店舗ID" = {sid}'))
        raw_count = raw_result.scalar()
        logging.warning(f"[ADMIN-PRINTERS] Raw SQL count: {raw_count}")
        for row in rows:
            logging.warning(f"[ADMIN-PRINTERS] Printer: id={row.id}, name={row.name}, kind={row.kind}, store_id={row.store_id}")
        printers = []
        for p in rows:
            # 印刷ルールで使用されているかチェック
            ...
            # 印刷設定を取得
            print_setting = s.query(PrinterFormatSetting).filter(
                PrinterFormatSetting.store_id == sid,
                PrinterFormatSetting.printer_name == p.name
            ).first()
            
            # 印刷モードを日本語で表示
            print_mode = "未設定"
            if print_setting:
                if print_setting.print_mode == "kitchen":
                    print_mode = "厨房モード"
                elif print_setting.print_mode == "receipt":
                    print_mode = "伝票モード"
...
# --- プリンタ新規登録（手動） ---------------------------------------------------
@bp.route("/admin/printers/new", methods=["POST"])
@require_admin
...
# --- プリンタ有効/無効トグル ----------------------------------------------------
@bp.route("/admin/printers/<int:pid>/toggle", methods=["POST"])
@require_admin
...
# --- プリンタ削除 --------------------------------------------------------------
@bp.route("/admin/printers/<int:pid>/delete", methods=["POST"])
@require_admin
...
# --- プリンタ編集 --------------------------------------------------------------
@bp.route("/admin/printers/<int:pid>/edit", methods=["POST"])
@require_admin
...
# --- プリンタ自動検出API（JSON） ------------------------------------------------
@bp.route("/admin/printers/discover", methods=["GET"], endpoint="admin_printers_discover")
@require_admin
...
# --- プリンタ一括登録API（JSON） ------------------------------------------------
@bp.route("/admin/printers/import", methods=["POST"], endpoint="admin_printers_import")
@require_admin
...
# --- APIキー生成 --------------------------------------------------------------
@bp.route("/admin/printers/generate-api-key", methods=["POST"])
@require_admin
...
# --- APIキー再生成 ----------------------------------------------------------
@bp.route("/admin/printers/regenerate-api-key", methods=["POST"])
@require_admin
...
def admin_rules():
    ...
    try:
        ...
        return render_template("pos/rules.html", title="印刷ルール",
...
# --- 印刷ルール新規作成 ---------------------------------------------------------
@bp.route("/admin/rules/new", methods=["POST"])
@require_admin
def admin_rules_new():
    f = request.form
    s = SessionLocal()
    try:
        ...
        try:
            ...
            s.rollback()
        return redirect(url_for("pos_app.admin_rules"))
    finally:
        s.close()
        SessionLocal.remove()
...
# --- 印刷ルール削除 -------------------------------------------------------------
@bp.route("/admin/rules/<int:rid>/delete", methods=["POST"])
@require_admin
...
@bp.route("/admin/dev/backfill_store_null")
@require_admin
...
def admin_member_new():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if errors:
                return render_template("pos/member_new.html", errors=errors, form={
        ...
        return render_template("pos/member_new.html", errors=None, form={
...
# --- 会計：単発完了（互換用） ---------------------------------------------------
@bp.route("/admin/settle/<int:table_id>", methods=["POST"])
@require_any
...
def invalidate_qr_tokens_for_table(s, table_id: int):
    ...
    try:
        ...
        current_current_app.logger.info(f"[QR_INVALIDATE] Deleted {deleted_count} tokens for table_id={table_id}")
        return deleted_count
    except Exception as e:
        current_current_app.logger.error(f"[QR_INVALIDATE] Failed to delete tokens for table_id={table_id}: {e}")
        return 0
...
def verify_token(token: str) -> int:
    ...
    try:
        ...
        if sign_payload(payload) != sig:
            current_current_app.logger.warning(f"[QR_VERIFY] Signature mismatch for {token}")
            return 0
        ...
        try:
            ...
            if not row:
                current_current_app.logger.info(f"[QR_VERIFY] Token not found in DB: {token}")
                return 0
            ...
            if row.expires_at:
                ...
                if exp_dt < now_utc:
                    current_current_app.logger.info(f"[QR_VERIFY] Token expired: {token}")
                    return 0
        ...
        current_current_app.logger.exception(f"[QR_VERIFY] Error during verification: {e}")
        return 0
...
@bp.route("/t/<tenant_slug>/table/<int:table_id>")
def menu_page_by_table(tenant_slug, table_id):
    ...
    try:
        ...
        # 3. 新しいトークンでリダイレクト
        current_current_app.logger.info(f"[QR_PERM] Generated new token for table={table_id}. Redirecting.")
        return redirect(url_for("pos_app.menu_page", tenant_slug=tenant_slug, token=new_token))
        
    except Exception as e:
        # current_app がない環境でのエラーを避けるため、try-exceptで囲む
        try:
            current_current_app.logger.exception(f"[QR_PERM] Error processing permanent QR for table={table_id}: {e}")
        except NameError:
            pass
...
# --- QRトークン発行（管理） -----------------------------------------------------
@bp.route("/admin/qrtoken/new", methods=["POST"])
@require_admin
def admin_new_token():
    ...
    if not token:
        ...
        if s is not None:
            try:
                ...
                current_current_app.logger.exception("[order_token] persist failed with provided session")
        else:
            ...
            try:
                ...
                current_current_app.logger.exception("[order_token] persist failed (short session)")
            finally:
                try:
                    s2.close()
                except Exception:
...
def _delete_order_if_empty(s: Session, header: "OrderHeader") -> bool:
    ...
    # デバッグログを出力
    current_current_app.logger.info(f"[_delete_order_if_empty] order_id={header.id}, items={debug_items}, active_qty_sum={active_qty_sum}")
    ...
    try:
        ...
        s.delete(header)
        s.commit()
        current_current_app.logger.info("[reset-session] deleted order header id=%s (empty/net-zero)", header.id)
        return True
    except Exception:
        s.rollback()
        current_current_app.logger.exception("[reset-session] failed to delete order header id=%s", header.id)
        return False
...
def _reset_customer_info_for_order(s: Session, header: "OrderHeader") -> None:
    ...
    try:
        ...
        current_current_app.logger.exception("[reset-session] delete T_お客様詳細 failed (best-effort)")
...
def _purge_customer_detail_for_table(s: Session, *, table_id: int):
    ...
    try:
        ...
        s.flush()
        current_current_app.logger.info("[reset-session] purged T_お客様詳細 by table_id=%s, n=%s", table_id, n)
    except Exception:
        current_current_app.logger.exception("[reset-session] purge by table_id failed")


# ---------------------------------------------------------------------
# セッション初期化（スタッフ以上専用）
# ---------------------------------------------------------------------
@bp.route("/orders/<int:order_id>/reset_session", methods=["POST"])
def reset_order_session(order_id: int):
    ...
    try:
        ...
        try:
            ...
            current_current_app.logger.exception("[reset-session] customer info reset failed")
        ...
        if wants_json:
            resp = make_response(jsonify({"ok": True, "redirect": url_for("pos_app.floor")}))
            for name in ("order_token", "customer_token", "guest_token"):
                resp.delete_cookie(name, path="/")
            return resp

        flash("未注文セッションを初期化しました。", "success")
        resp = redirect(url_for("pos_app.floor"))
        for name in ("order_token", "customer_token", "guest_token"):
            resp.delete_cookie(name, path="/")
        return resp
...
@bp.route("/t/<tenant_slug>/m/<token>")
    ...
    try:
        ...
        if header and (getattr(header, "status", "") == "会計済" or getattr(header, "closed_at", None)):
            ...
            if request.cookies.get("order_token"):
                resp = make_response(redirect(url_for("pos_app.menu_page", tenant_slug=tenant_slug, token=token)))
                resp.delete_cookie("order_token")
                current_current_app.logger.info(f"[MENU_PAGE] Deleted order_token for closed order: {header.id}")
                return resp
        ...
        if is_staff_or_higher():
            ...
            resp = make_response(render_template("pos/menu_page.html", **template_vars))
        ...
        # B) ヘッダは存在するが、この端末はまだ注文に参加していない場合
            ...
            if not require_pin:
                resp = make_response(
                    redirect(url_for("pos_app.menu_page", tenant_slug=tenant_slug, token=token))
                )
                return _issue_order_token_if_needed(resp, header, s)
...
def api_order():
    ...
    if custom_date_str:
        current_app.logger.info(f"[api_order] 過去日付モード: custom_date={custom_date_str}")
    ...
    table_id = verify_token(token)
    current_app.logger.info(f"DEBUG: verify_tokenから取得したtable_id: {table_id}")
    ...
    try:
        ...
        if not t:
            ...
            current_app.logger.warning(f"table not found for table_id: {table_id}")
        ...
        if store_id is None:
            current_app.logger.error("[api_order] store_id could not be resolved (table_id=%s)", table_id)
        ...
        if not order:
            ...
            try:
                ...
                current_app.logger.debug("[api_order] could not set table status", exc_info=True)
            s.flush()  # ← order.id を確定（ここで NOT NULL 違反していた）
            created_new_order = True
            current_app.logger.info("[api_order] created new order id=%s (table_id=%s, store_id=%s, tenant_id=%s)",
            ...
            try:
                ...
                current_app.logger.debug("[api_order] could not backfill store/tenant for existing header", exc_info=True)

            current_app.logger.info("[api_order] reuse active order id=%s (table_id=%s)", order.id, table_id)
        ...
        try:
            ...
            if TCustomerDetail is None:
                current_app.logger.warning("[api_order] TCustomerDetail model not found (T_お客様詳細 / M_顧客詳細 / CustomerDetail)")
            else:
                current_app.logger.debug(
                ...
                if hasattr(TCustomerDetail, "order_id"):
                    ...
                    current_app.logger.debug("[api_order] search by order_id=%s -> %s", order.id, "hit" if cd else "none")
                else:
                    current_app.logger.warning("[api_order] TCustomerDetail has no order_id column!")
                ...
                if cd is None and hasattr(TCustomerDetail, "table_id"):
                    ...
                    if hasattr(TCustomerDetail, "order_id"):
                        ...
                        current_app.logger.debug("[api_order] searching orphan rows for table_id=%s", table_id)
                    cd = q.order_by(getattr(TCustomerDetail, "id").asc()).first()
                    current_app.logger.debug("[api_order] orphan search -> %s", "hit" if cd else "none")
                ...
                if cd is None:
                    ...
                    for col in ("大人男性", "大人女性", "子ども男", "子ども女", "合計人数"):
                        if hasattr(TCustomerDetail, col):
                            setattr(cd, col, 0)
                    s.add(cd)
                    current_app.logger.info("[api_order] created new customer_detail (table_id=%s)", table_id)
                ...
                if hasattr(TCustomerDetail, "order_id"):
                    ...
                    current_app.logger.debug("[api_order] set cd.order_id: %s -> %s", before, after)
                else:
                    current_app.logger.warning("[api_order] cannot set order_id (column not present)")
            ...
            current_app.logger.exception("[api_order] bind TCustomerDetail failed: %s", e)
        # ========= 顧客詳細紐付け ここまで =========

        # --- 明細作成に使う store_id は、上で確定した値をそのまま使う ---
        current_app.logger.debug("[api_order] store_id for order items: %s", store_id)
        ...
        for it in items:
            try:
                ...
                current_app.logger.debug("[api_order] skip item (invalid menu_id/qty): %s", it)
                continue
            if qty <= 0:
                current_app.logger.debug("[api_order] skip item (qty<=0): %s", it)
                continue
            ...
            m = menu_map.get(mid)
            if not m:
                current_app.logger.debug("[api_order] skip item (menu not available): id=%s", mid)
                continue
            ...
            if actual_price is not None:
                ...
                current_app.logger.info("[api_order] market price item: menu_id=%s actual_price=%s", mid, unit)
            ...
            if custom_date_str:
                try:
                    ...
                    current_app.logger.info(f"[api_order] 過去日付で登録: added_at={added_at_value}, status={item_status}, scheduled_date={scheduled_date_value}")
                except Exception as parse_err:
                    current_app.logger.warning("[api_order] custom_date parse failed: %s", parse_err)
            ...
            try:
                ...
                current_app.logger.warning("[api_order] progress_seed failed for item %s: %s", new_item.id, e)
        ...
        if added == 0:
            current_app.logger.warning("[api_order] no valid items -> rollback")
            s.rollback()
        ...
        current_app.logger.debug("[api_order] commit: order_id=%s subtotal=%s tax=%s total=%s",
        ...
        try:
            ...
            current_app.logger.exception("[api_order] KDS print failed (non-fatal)")
        ...
        s.rollback()
        current_app.logger.error("[api_order] error: %s", e, exc_info=True)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def _record_table_move_history(s, sid, from_table_id, to_table_id, mode, order_from, order_to=None, new_order_id=None):
    ...
    try:
        ...
        if THistory is None:
            current_current_app.logger.warning("[table_move_history] T_テーブル移動履歴 not found")
            return None
        ...
        current_current_app.logger.info("[table_move_history] recorded: id=%s from=%s to=%s mode=%s order=%s dest_order=%s new_order=%s",
        ...
        current_current_app.logger.exception("[table_move_history] failed to record: %s", e)
        # 履歴記録失敗は致命的ではないので続行
...
@bp.route("/admin/table/move", methods=["POST"])
@require_staff
def admin_table_move():
        ...
        try:
            if hasattr(Qr, "issued_at"):
                ...
                latest = (s.query(Qr)
                            ...
                            .filter(Qr.store_id == store_id).first())
            if latest:
                # 修正: QRトークンのtable_idは変更しない（QRコードの紐付けを保持）
                # changed = {"id": getattr(latest, "id", None), "before": from_table_id, "after": to_table_id}
                # latest.table_id = to_table_id
                pass
        except Exception:
            current_current_app.logger.exception("[table_move] QR rebind failed (from=%s to=%s)", from_table_id, to_table_id)
        return changed
    ...
    def _reset_customer_detail_for_table(TCD, table_id):
        ...
        by_table = s.query(TCD).filter(TCD.table_id == table_id)\
                   .delete(synchronize_session=False)
        current_current_app.logger.info("[table_move][cleanup] table_id=%s -> orphans=%s, by_table=%s",
    ...
    def _coalesce_customer_detail(TCD, *, order_id: int, table_id: int):
        ...
        if not numeric_cols:
            current_current_app.logger.warning("[table_move][coalesce] no numeric columns found on T_お客様詳細")
            return {"rows": 0, "into": None, "sums": {}}
        ...
        if h:
            ...
            for attr in ("total", "合計"):
                if hasattr(TOrder, attr):
                    setattr(h, attr, tot)
            s.flush()
            current_current_app.logger.info("[table_move][recalc] order=%s subtotal=%s tax=%s total=%s",
    ...
    try:
        ...
        s.rollback()
        current_current_app.logger.exception("admin_table_move failed")
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/admin/table_move_history")
@require_store_admin
...
@bp.route("/staff/order/<int:table_id>")
@require_staff
def staff_order(table_id):
    ...
    sid = current_store_id()
    if sid is None:
        return redirect(url_for("pos_app.staff_login"))
...
@bp.route("/staff/table/<int:table_id>")
@require_staff
def staff_table(table_id: int):
    # 互換用：旧エンドポイントに来たら、menu_page へ橋渡しする新ルートへ転送
    return redirect(url_for('pos_app.staff_open_menu', table_id=table_id))
...
@bp.route("/staff/open/<int:table_id>/menu", endpoint="staff_open_menu")
@require_staff
def staff_open_menu(table_id: int):
    ...
    try:
        ...
        return redirect(url_for("pos_app.menu_page", tenant_slug=tenant_slug, token=qt.token))
...
@bp.route("/staff/api/order", methods=["POST"])
@require_staff
def staff_api_order():
    """
    ...
    # デバッグ: custom_dateの受信確認
    if custom_date_str:
        current_app.logger.info(f"[staff_api_order] 過去日付モード: custom_date={custom_date_str}")
    ...
    try:
        ...
        # ★ デバッグ：テーブル情報
        try:
            current_app.logger.debug("[staff_api_order] table_id=%s store_id=%s status=%s",
        ...
        if TCustomerDetail is None:
            current_app.logger.warning("[staff_api_order] TCustomerDetail model not found (T_お客様詳細 / M_顧客詳細 / CustomerDetail)")
        else:
            current_app.logger.debug(
        ...
        try:
            ...
            store_id = None
        current_app.logger.debug("[staff_api_order] resolved store_id=%s", store_id)
        ...
        current_app.logger.debug("[staff_api_order] active order found? %s", bool(order))
        ...
        if not order:
            ...
            try:
                ...
                current_app.logger.debug("[staff_api_order] table status set failed (non-fatal)")
            s.flush()  # order.id を確定
            new_order_created = True
            current_app.logger.info("[staff_api_order] created new order id=%s for table_id=%s", order.id, table_id)
        else:
            current_app.logger.info("[staff_api_order] reuse active order id=%s for table_id=%s", order.id, table_id)
        ...
        if TCustomerDetail is not None:
            try:
                ...
                if hasattr(TCustomerDetail, "order_id"):
                    ...
                    current_app.logger.debug("[staff_api_order] search by order_id=%s -> %s", order.id, "hit" if cd else "none")
                else:
                    current_app.logger.warning("[staff_api_order] TCustomerDetail has no order_id column!")
                ...
                if cd is None:
                    ...
                    if hasattr(TCustomerDetail, "order_id"):
                        q = q.filter(getattr(TCustomerDetail, "order_id") == None)  # noqa: E711
                        current_app.logger.debug("[staff_api_order] searching orphan rows for table_id=%s", table_id)
                    cd = q.order_by(getattr(TCustomerDetail, "id").asc()).first()
                    current_app.logger.debug("[staff_api_order] search orphan by table_id=%s -> %s", table_id, "hit" if cd else "none")
                ...
                if cd is None:
                    ...
                    for col in ("大人男性", "大人女性", "子ども男", "子ども女", "合計人数"):
                        if hasattr(TCustomerDetail, col):
                            setattr(cd, col, 0)
                    s.add(cd)
                    current_app.logger.info("[staff_api_order] created new TCustomerDetail (table_id=%s)", table_id)
                ...
                if hasattr(TCustomerDetail, "order_id"):
                    ...
                    setattr(cd, "order_id", order.id)
                    after = getattr(cd, "order_id", None)
                    current_app.logger.debug("[staff_api_order] set cd.order_id: %s -> %s", before, after)
                else:
                    current_app.logger.warning("[staff_api_order] cannot set order_id because column not present")
                ...
                current_app.logger.info("[staff_api_order] bound customer_detail id=%s to order=%s (created_new=%s)",
                                bound_cd_id, order.id, created_new_cd)
            except Exception as e:
                current_app.logger.exception("[staff_api_order] bind TCustomerDetail failed: %s", e)
        ...
        for it in items:
            ...
            try:
                ...
                current_app.logger.debug("[staff_api_order] skip item (invalid menu_id/qty): %s", it)
                continue
            if qty <= 0:
                current_app.logger.debug("[staff_api_order] skip item (qty<=0): %s", it)
                continue
            ...
            if not (m and m.available == 1):
                current_app.logger.debug("[staff_api_order] skip item (menu not available): id=%s", mid)
                continue
            ...
            if actual_price is not None:
                unit = int(actual_price)
                current_app.logger.info("[staff_api_order] market price item: menu_id=%s actual_price=%s", mid, unit)
            ...
            if custom_date_str:
                try:
                    ...
                    date_memo = f"[登録予定: {custom_date_str}]"
                    memo = f"{date_memo} {memo}".strip() if memo else date_memo
                    current_app.logger.info(f"[staff_api_order] 過去日付で登録: added_at={added_at_value}, status={item_status}, scheduled_date={scheduled_date_value}")
                except Exception as parse_err:
                    current_app.logger.warning("[staff_api_order] custom_date parse failed: %s", parse_err)
                    added_at_value = datetime.now()
                    item_status = "新規"
                    scheduled_date_value = None
            else:
        ...
        if added == 0:
            current_app.logger.warning("[staff_api_order] no valid items (rollback). new_order_created=%s", new_order_created)
            s.rollback()
            return jsonify({"ok": False, "error": "no valid items"}), 400
        ...
        # ★ コミット前に最終ログ
        current_app.logger.debug("[staff_api_order] commit: order_id=%s subtotal=%s tax=%s total=%s cd_id=%s",
        ...
        # except Exception:
        #     current_app.logger.exception("[staff_api_order] failed to print")
        ...
        s.rollback()
        current_app.logger.error("[staff_api_order] error: %s", e, exc_info=True)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def _progress_update_core(item_id: int):
    ...
    s = SessionLocal()
    try:
        current_app.logger.info("[DEBUG-CANCEL] Step 1: Starting _progress_update_core for item_id=%s", item_id)
        j = request.get_json(force=True) or {}
        raw_status = (j.get("status") or "").strip()
        count  = int(j.get("count") or 1)
        current_app.logger.info("[DEBUG-CANCEL] Step 2: raw_status=%s, count=%s", raw_status, count)
        if count <= 0:
            return jsonify(ok=False, error="count must be >= 1"), 400

        action = _norm_status(raw_status)
        current_app.logger.info("[DEBUG-CANCEL] Step 3: action=%s", action)
        if action not in {"cooking","served","cancel","new"}:
            return jsonify(ok=False, error="invalid status"), 400

        OrderItem, Menu = _models()
        current_app.logger.info("[DEBUG-CANCEL] Step 4: _models() returned OrderItem=%s, Menu=%s", OrderItem, Menu)
        it = s.get(OrderItem, item_id)
        current_app.logger.info("[DEBUG-CANCEL] Step 5: Got item=%s", it)
        if not it:
            return jsonify(ok=False, error="item not found"), 404

        # 進捗行シード（未作成なら qty_new=元数量）
        current_app.logger.info("[DEBUG-CANCEL] Step 6: Calling progress_seed_if_needed")
        progress_seed_if_needed(s, it)

        # 取消時は負行作成に必要な税率を先に確保
        tax_rate = None
        if action == "cancel":
            current_app.logger.info("[DEBUG-CANCEL] Step 7: action is cancel, getting tax_rate")
            menu_id = _get_any(it, "menu_id", "メニューid", "商品id")
            menu = s.get(Menu, menu_id) if menu_id is not None else None
            tax_rate = _guess_tax_rate(src_item=it, menu=menu)
            current_app.logger.info("[DEBUG-CANCEL] Step 8: tax_rate=%s", tax_rate)

        # 実移動
        try:
            current_app.logger.info("[DEBUG-CANCEL] Step 9: Calling progress_move")
            p_after, moved = progress_move(s, it, action, count)
            current_app.logger.info("[DEBUG-CANCEL] Step 10: progress_move returned p_after=%s, moved=%s", p_after, moved)
        except ValueError as e:
            current_app.logger.error("[DEBUG-CANCEL] ValueError in progress_move: %s", e)
            s.rollback()
            return jsonify(ok=False, error=str(e)), 400
        except Exception as e:
            current_app.logger.exception("[DEBUG-CANCEL] Exception in progress_move: %s", e)
            s.rollback()
        ...
        if action == "cancel" and moved > 0:
            current_app.logger.info("[DEBUG-CANCEL] Step 11: Creating negative item, moved=%s", moved)
            # 実際に動いた数だけマイナス行を作る
            neg = OrderItem()
            current_app.logger.info("[DEBUG-CANCEL] Step 12: Created OrderItem instance")
            _copy_if_exists(neg, it, [
                (["order_id","注文id","注文ID"], ["order_id","注文id","注文ID"]),
                (["menu_id","メニューid","商品id"], ["menu_id","メニューid","商品id"]),
                (["store_id","店舗ID"], ["store_id","店舗ID"]),
            ...
            if hasattr(neg, "追加日時"):   setattr(neg, "追加日時", now)
            if hasattr(neg, "added_at"):   neg.added_at = now
            current_app.logger.info("[DEBUG-CANCEL] Step 13: Adding negative item to session")
            s.add(neg); 
            current_app.logger.info("[DEBUG-CANCEL] Step 14: Flushing session")
            s.flush()
            neg_id = getattr(neg, "id", None)
            current_app.logger.info("[DEBUG-CANCEL] Step 15: neg_id=%s", neg_id)

        # 自動確定（提供済+取消 == 元数量 → 明細.status=提供済）
        current_app.logger.info("[DEBUG-CANCEL] Step 16: Calling progress_finalize_if_done")
        finalized = progress_finalize_if_done(s, it)
        current_app.logger.info("[DEBUG-CANCEL] Step 17: finalized=%s", finalized)

        current_app.logger.info("[DEBUG-CANCEL] Step 18: Committing transaction")
        s.commit()
        current_app.logger.info("[DEBUG-CANCEL] Step 19: Calling mark_floor_changed")
        mark_floor_changed()
        current_app.logger.info("[DEBUG-CANCEL] Step 20: Success, returning response")
...
@bp.route("/staff/api/order/<int:order_id>/status", methods=["POST"])
...
def staff_update_order_status(order_id):
    ...
    try:
        ...
        s.rollback()
        current_app.logger.exception("[staff_update_order_status] %s", e)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def menu_page_legacy(token=None):
    ...
    if not slug:
        current_app.logger.warning("[menu_page_legacy] tenant_slug missing; session=%r g.tenant=%r",
                           dict(session), getattr(g, "tenant", None))
        return "tenant_slug が不明です。ログインし直してください。", 400

    # エンドポイント 'menu_page' に合わせて転送
    try:
        return redirect(url_for("pos_app.menu_page", tenant_slug=slug, token=token))
    except Exception as e:
        current_app.logger.error("[menu_page_legacy] url_for(menu_page, tenant_slug=%s, token=%s) failed: %s",
                         slug, token, e)
...
<style>body{font-family:sans-serif;margin:20px}</style>
  ...
  <p><a href="{{ url_for('pos_app.floor') }}">フロアへ戻る</a></p>
</body></html>
...
@bp.route("/reports/sales", endpoint="sales_report")
@require_store_admin
def sales_report():
    s = SessionLocal()
    try:
        ...
        if Hist and order_ids:
            ...
            current_app.logger.debug("[sales_report] Hist rows scanned: %s", len(rows))
            ...
            if not Hist:
                ...
                dbg["note"].append(msg)
                current_app.logger.debug("[sales_report] %s", msg)
            if not order_ids:
                current_app.logger.debug("[sales_report] 対象注文が 0 件です。")
        ...
        current_app.logger.debug("[sales_report] guests_total=%s / orders=%s", guests_total, len(orders))
        ...
        current_app.logger.debug(f"[sales_report] 過去日付明細: {past_date_count}, 通常明細: {normal_count}")
...
# API: アクティブな過去日付注文の確認
# ---------------------------------------------------------------------
@bp.route("/api/orders/active_past_orders")
def api_check_active_past_orders():
    ...
    try:
        ...
        current_app.logger.error(f"[api_check_active_past_orders] error: {e}", exc_info=True)
        return jsonify({
            "ok": False,
...
def api_sales_products():
    ...
    try:
        ...
        qs = ""
    current_current_app.logger.info("[/api/sales/products] ENTER q=%s", qs)
    ...
    try:
        ...
        if debug_mode:
            current_current_app.logger.info("[/api/sales/products] rows=%s sid=%s period=%s..%s",
        ...
        current_current_app.logger.exception("[/api/sales/products] ERROR: %s", e)
...
@bp.route("/api/sales/payment_methods")
@require_store_admin
...
@bp.route("/api/sales/export/daily")
@require_store_admin
...
@bp.route("/api/sales/export/products")
@require_store_admin
...
@bp.route("/api/staff_call", methods=["POST"])
def api_staff_call():
    """
    お客様が店員を呼び出すAPI
    POST JSON:
    ...
    try:
        ...
        try:
            ...
            current_app.logger.warning(f"[api_staff_call] token validation warning: {e}")
        ...
        current_app.logger.info(f"[STAFF_CALL] テーブル {table_no} から店員呼び出し (timestamp={new_timestamp}, deleted_confirmed={deleted}, total_pending={total_calls})")
        
        return jsonify({"ok": True, "table_no": table_no, "timestamp": new_timestamp})
    
    except Exception as e:
        s.rollback()
        current_app.logger.error(f"[api_staff_call] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()


@bp.route("/api/staff_call/poll", methods=["GET"])
@require_any
def api_staff_call_poll():
    ...
    try:
        ...
        if calls:
            current_app.logger.info(f"[STAFF_CALL_POLL] Returning {len(calls)} calls: {calls}")
        
        return jsonify({"ok": True, "calls": calls})
    
    except Exception as e:
        current_app.logger.error(f"[api_staff_call_poll] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()


@bp.route("/api/staff_call/confirm", methods=["POST"])
@require_any
def api_staff_call_confirm():
    ...
    try:
        ...
        if call:
            ...
            current_app.logger.info(f"[STAFF_CALL_CONFIRM] テーブル {table_no} の呼び出しを確認 (timestamp={timestamp}, remaining_calls={remaining})")
        else:
            current_app.logger.warning(f"[STAFF_CALL_CONFIRM] テーブル {table_no} の呼び出しが見つかりません (timestamp={timestamp})")
        
        return jsonify({"ok": True})
    
    except Exception as e:
        s.rollback()
        current_app.logger.error(f"[api_staff_call_confirm] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()


@bp.route("/api/debug_log", methods=["POST"])
def api_debug_log():
    """
    ...
    try:
        ...
        current_app.logger.error(f"[api_debug_log] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
...
# プリンター設定API（ブラウザ側印刷用）
# =========================================================
@bp.route("/api/printer_config", methods=["GET"])
def api_printer_config():
    ...
    try:
        ...
        # 店舗の printer_server_url を取得
        store = s.get(Store, sid)
        if store and store.printer_server_url:
            return jsonify({"printer_server_url": store.printer_server_url})
        else:
            return jsonify({"printer_server_url": None})
    except Exception as e:
        current_app.logger.error(f"[api_printer_config] error: {e}", exc_info=True)
...
@bp.route("/api/print_data/<int:order_id>", methods=["GET"])
def api_print_data(order_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"[api_print_data] error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# メニューオプション取得API
# =========================================================
@bp.route("/api/menu/<int:menu_id>/options")
def api_menu_options(menu_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"[api_menu_options] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# =========================================================
@bp.route("/admin/order/<int:order_id>/reopen", methods=["POST"])
@require_store_admin
    ...
    try:
        ...
        for f in ("closed_at", "精算日時", "closedAt"):
            if hasattr(h, f):
                try:
                    ...
                    current_app.logger.debug("[admin_reopen_order] clear %s failed", f)
        ...
        if t is not None and hasattr(t, "status"):
            try:
                ...
                current_app.logger.debug("[admin_reopen_order] set TableSeat.status failed")
        ...
        if "restore_customer_detail_from_history" in globals():
            try:
                ...
                current_app.logger.exception("[admin_reopen_order] restore header guests failed: %s", e)
        ...
        if "_upsert_guest_detail_from_history" in globals():
            try:
                ...
                current_app.logger.exception("[admin_reopen_order] upsert current guests failed: %s", e)
        ...
        try:
            ...
            current_app.logger.exception("[admin_reopen_order] recalc totals (including negatives) failed: %s", e)
        ...
        s.rollback()
        current_app.logger.exception("[admin_reopen_order] %s", e)
        return jsonify({"ok": False, "error": "internal error"}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/sysadmin/store_master")
@require_sysadmin
def store_master_list():
    ...
    try:
        ...
        return render_template("pos/store_master_list.html", stores=stores)
    finally:
        s.close()
        SessionLocal.remove()
...
@bp.route("/sysadmin/store_master/add", methods=["GET", "POST"])
@require_sysadmin
    ...
    try:
        if request.method == "POST":
            ...
            if not store_code or not store_name:
                ...
                return render_template("pos/store_master_form.html")
            ...
            return redirect(url_for("pos_app.store_master_list"))
...
@bp.route("/sysadmin/store_master/<int:store_id>/edit", methods=["GET", "POST"])
@require_sysadmin
def store_master_edit(store_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not store_code or not store_name:
                flash("店舗コードと店舗名は必須です。")
                return render_template("pos/store_master_form.html", store=store)
...
@bp.route("/sysadmin/store_master/<int:store_id>/delete", methods=["POST"])
@require_sysadmin
...
def dev_tools():
    ...
    try:
        ...
        return render_template("pos/dev_tools.html", stats=stats)
    finally:
        s.close()
        SessionLocal.remove()
...
def _reset_customer_detail_after_checkout(s, *, order_id=None, table_id=None) -> dict:
    ...
    Model = globals().get("T_お客様詳細")
    if Model is None:
        current_current_app.logger.warning("[reset_customer_detail] T_お客様詳細 not found")
    ...
    try:
        ...
        current_current_app.logger.exception("[reset_customer_detail][peek] failed: %s", e)
    ...
    try:
        ...
        if not insp.has_table("T_お客様詳細"):
            try:
                ...
                current_app.logger.debug('[migrate] created table T_お客様詳細')
            except Exception as e:
                current_app.logger.exception('[migrate] create T_お客様詳細 failed: %s', e)
                raise
        ...
        for name, typ, default in add_cols:
            try:
                ...
                current_app.logger.debug(f'[migrate] add column T_お客様詳細.{name}')
            except Exception as e:
                current_app.logger.exception('[migrate] add column %s failed: %s', name, e)
        # index は必要に応じて
    finally:
        s.close()
        SessionLocal.remove()
...
# テーブル移動取消のメイン関数
# ---------------------------------------------------------------------
@bp.route("/admin/table/move/cancel", methods=["POST"])
@require_staff
def admin_table_move_cancel():
    ...
    try:
        ...
        current_current_app.logger.info("[table_move_cancel] history_id=%s mode=%s restored=%s",
        ...
        s.rollback()
        current_current_app.logger.exception("[table_move_cancel] failed")
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# テーブル移動取消可能チェックAPI
# ---------------------------------------------------------------------
@bp.route("/admin/table/move/cancel_check/<int:history_id>", methods=["GET"])
@require_staff
def admin_table_move_cancel_check(history_id):
    ...
    try:
        ...
        current_current_app.logger.exception("[table_move_cancel_check] failed")
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def __menu_test():
    ...
    try:
        ...
        current_app.logger.info("[__menu_test] sid=%s tid=%s -> %s menus", sid, tid, len(menus))
        return render_template("pos/menu_page.html", menus=menus)
    except Exception as e:
        current_app.logger.exception("__menu_test failed")
        return jsonify({"error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
def __menu_diag():
    ...
    try:
        ...
        current_app.logger.exception("__menu_diag failed")
        return jsonify({"error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# ========================================
@bp.route("/api/printer-server/new-orders", methods=["GET"])
def api_printer_server_new_orders():
    ...
    # 詳細なログ出力
    print("[PRINTER-SERVER-API] === 新規リクエスト ===")
    print(f"[PRINTER-SERVER-API] Full URL: {request.url}")
    print(f"[PRINTER-SERVER-API] Query String: {request.query_string.decode('utf-8')}")
    print(f"[PRINTER-SERVER-API] All Args: {dict(request.args)}")
    
    store_id = request.args.get("store_id", type=int)
    print(f"[PRINTER-SERVER-API] store_id: {store_id}")
    if not store_id:
        return jsonify({"ok": False, "error": "store_id is required"}), 400
    
    provided_api_key = request.args.get("api_key", "")
    print(f"[PRINTER-SERVER-API] provided_api_key: '{provided_api_key}'")
    print(f"[PRINTER-SERVER-API] provided_api_key length: {len(provided_api_key)}")
    print(f"[PRINTER-SERVER-API] provided_api_key type: {type(provided_api_key)}")
    print(f"[PRINTER-SERVER-API] provided_api_key repr: {repr(provided_api_key)}")
    
    # APIキー認証（データベースから取得）
    # テナントフィルタを完全にバイパスするため、生SQLを使用
    s_auth = SessionLocal()
    try:
        # 生SQLでStoreを取得（テナントフィルタを回避）
        result = s_auth.execute(
            text('SELECT id, "プリンターサーバーAPIキー" FROM "M_店舗" WHERE id = :store_id'),
            {"store_id": store_id}
        ).fetchone()
        ...
        store_id_db, expected_api_key = result
        
        print(f"[PRINTER-SERVER-API] expected_api_key: '{expected_api_key}'")
        print(f"[PRINTER-SERVER-API] expected_api_key length: {len(expected_api_key) if expected_api_key else 0}")
        print(f"[PRINTER-SERVER-API] expected_api_key type: {type(expected_api_key)}")
        print(f"[PRINTER-SERVER-API] expected_api_key repr: {repr(expected_api_key)}")
        
        if not expected_api_key:
            print("[PRINTER-SERVER-API] ERROR: API key not configured")
            return jsonify({"ok": False, "error": "API key not configured for this store"}), 401
        
        print(f"[PRINTER-SERVER-API] Comparing: '{provided_api_key}' == '{expected_api_key}'")
        print(f"[PRINTER-SERVER-API] Match result: {provided_api_key == expected_api_key}")
        
        if provided_api_key != expected_api_key:
            print("[PRINTER-SERVER-API] ERROR: API key mismatch")
            return jsonify({"ok": False, "error": "Invalid API key"}), 401
        
        print("[PRINTER-SERVER-API] SUCCESS: API key matched")
    finally:
        s_auth.close()
    
    # since_item_id をサポート（OrderItem 単位で管理）
    since_item_id = request.args.get("since_item_id", type=int, default=0)
    # 後方互換性のため、古い since_id もサポート
    if since_item_id == 0:
        since_item_id = request.args.get("since_id", type=int, default=0)
    print(f"[PRINTER-SERVER-API] since_item_id (final): {since_item_id}")
    
    # init=true の場合は最新のitem_idのみ返す（初回起動時の初期化用）
    init_mode = request.args.get("init", "").lower() == "true"
    if init_mode:
        s_init = SessionLocal()
        try:
            latest = s_init.execute(
                text('SELECT MAX(id) FROM "T_注文明細" WHERE "店舗ID" = :store_id'),
                {"store_id": store_id}
            ).scalar()
            latest_item_id = latest or 0
            print(f"[PRINTER-SERVER-API] init mode: latest_item_id={latest_item_id}")
            return jsonify({"ok": True, "items": [], "latest_item_id": latest_item_id})
        finally:
            s_init.close()
    
    # print_type: 'kitchen'=厨房印刷済みフィルター, 'receipt'=明細印刷済みフィルター, 未指定=従来のsince_item_id方式
    print_type = request.args.get("print_type", "").lower()  # 'kitchen' or 'receipt'
    
    s = SessionLocal()
    try:
        # テナントフィルタを完全にバイパスするため、生 SQL を使用
        # 印刷済みフラグがあればそれでフィルター、なければ従来のsince_item_id方式
        if print_type == 'kitchen':
            # 厨房印刷済み=falseのアイテムのみ取得（印刷済みカラムが存在する場合）
            filter_sql = '''
                SELECT 
                    i.id,
                    i."\u6ce8\u6587ID",
                    ...
                    i."\u8ffd\u52a0\u65e5\u6642",
                    o."\u30c6\u30fc\u30d6\u30ebID",
                    o."\u72b6\u614b" as order_status
                FROM "T_\u6ce8\u6587\u660e\u7d30" i
                JOIN "T_\u6ce8\u6587" o ON i."\u6ce8\u6587ID" = o.id
                WHERE i."\u5e97\u8217ID" = :store_id
                  AND (i."厨房印刷済み" IS NULL OR i."厨房印刷済み" = false)
                  AND (o."\u72b6\u614b" IS NULL OR o."\u72b6\u614b" NOT IN ('\u4f1a\u8a08\u6e08', 'closed', 'paid'))
                ORDER BY i.id ASC
            '''
            result = s.execute(text(filter_sql), {"store_id": store_id}).fetchall()
        elif print_type == 'receipt':
            # 明細印刷済み=falseのアイテムのみ取得
            filter_sql = '''
                SELECT 
                    i.id,
                    i."\u6ce8\u6587ID",
                ...
                JOIN "T_\u6ce8\u6587" o ON i."\u6ce8\u6587ID" = o.id
                WHERE i."\u5e97\u8217ID" = :store_id
                  AND (i."\u660e\u7d30\u5370\u5237\u6e08\u307f" IS NULL OR i."\u660e\u7d30\u5370\u5237\u6e08\u307f" = false)
                  AND (o."\u72b6\u614b" IS NULL OR o."\u72b6\u614b" NOT IN ('\u4f1a\u8a08\u6e08', 'closed', 'paid'))
                ORDER BY i.id ASC
            '''
            result = s.execute(text(filter_sql), {"store_id": store_id}).fetchall()
        else:
            # 従来方式（since_item_idベース）
            result = s.execute(
                text('''
                    SELECT 
                        i.id,
                    ...
                    JOIN "T_注文" o ON i."注文ID" = o.id
                    WHERE i."店舗ID" = :store_id
                      AND i.id > :since_item_id
                      AND (o."状態" IS NULL OR o."状態" NOT IN ('会計済', 'closed', 'paid'))
                    ORDER BY i.id ASC
                '''),
                {"store_id": store_id, "since_item_id": since_item_id}
            ).fetchall()
        
        print(f"[PRINTER-SERVER-API] Query returned {len(result)} items")
        
        # デバッグ: 最近の OrderItem を確認
        all_recent = s.execute(
            text('SELECT id, "注文ID" FROM "T_注文明細" WHERE "店舗ID" = :store_id ORDER BY id DESC LIMIT 5'),
            {"store_id": store_id}
        ).fetchall()
        print(f"[PRINTER-SERVER-API] Recent 5 items: {[(r[0], r[1]) for r in all_recent]}")
        
        print(f"[PRINTER-SERVER-API] Filtered items: {[r[0] for r in result]}")
        
        items = []
        for row in result:
            item_id, order_id, menu_id, qty, added_at, table_id, order_status = row
            
            table_name = "不明"
            if table_id:
                # テーブル番号を取得（生 SQL）
                table_result = s.execute(
                    text('SELECT "テーブル番号" FROM "M_テーブル" WHERE id = :table_id'),
                    {"table_id": table_id}
                ).fetchone()
                if table_result:
                    table_name = table_result[0] or f"Table {table_id}"
            
            # メニュー名を取得
            menu_name = "不明"
            menu_result = s.execute(
                text('SELECT "名称" FROM "M_メニュー" WHERE id = :menu_id'),
                {"menu_id": menu_id}
            ).fetchone()
            if menu_result:
                menu_name = menu_result[0] or f"Menu {menu_id}"
        ...
        current_app.logger.error(f"[api_printer_server_new_orders] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# ========================================
# mark-printed API（印刷済みフラグ更新）
# ========================================
@bp.route("/api/printer-server/mark-printed", methods=["POST"])
def api_printer_server_mark_printed():
    """
    印刷済みフラグを更新する
    POST /api/printer-server/mark-printed
    Body: { "store_id": ..., "api_key": ..., "item_ids": [...], "print_type": "kitchen" or "receipt" }
    """
    ...
    print_type = (data.get("print_type") or "").lower()  # 'kitchen' or 'receipt'
    
    if not store_id or not api_key:
        return jsonify({"ok": False, "error": "store_id and api_key required"}), 400
    if not item_ids:
        return jsonify({"ok": True, "updated": 0, "message": "no item_ids provided"})
    if print_type not in ('kitchen', 'receipt'):
        return jsonify({"ok": False, "error": "print_type must be 'kitchen' or 'receipt'"}), 400
    ...
    try:
        ...
        if not store_row:
            return jsonify({"ok": False, "error": "Store not found"}), 404
        _, expected_api_key = store_row
        if expected_api_key != api_key:
            return jsonify({"ok": False, "error": "invalid api_key"}), 403
        
        # 印刷済みカラムを死殺防止のため存在確認
        col_name = "厨房印刷済み" if print_type == 'kitchen' else "明細印刷済み"
        col_check = s.execute(
            text("""
                SELECT column_name FROM information_schema.columns
                WHERE table_name = 'T_\u6ce8\u6587\u660e\u7d30'
        ...
        if not col_check:
            # カラムがまだ存在しない場合（マイグレーション前）はエラーを返す
            return jsonify({"ok": False, "error": f"column '{col_name}' not found in T_\u6ce8\u6587\u660e\u7d30. Migration may be needed."}), 500
        ...
        result = s.execute(update_sql, {"item_ids": item_ids, "store_id": store_id})
        s.commit()
        
        updated_count = result.rowcount
        current_app.logger.info(f"[mark-printed] {print_type}: updated {updated_count} items: {item_ids}")
        
        return jsonify({"ok": True, "updated": updated_count, "print_type": print_type})
    
    except Exception as e:
        s.rollback()
        current_app.logger.error(f"[api_printer_server_mark_printed] error: {e}", exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
    finally:
        s.close()
        SessionLocal.remove()
...
# ========================================
@bp.route("/download/printer-server")
...
# ========================================
@bp.route("/download/printer-server-exe")
def download_printer_server_exe():
    """
    printer-server Electron版のインストーラー(.exe)をZIP形式でダウンロード
    
    注意: このエンドポイントは、事前にビルドされたZIPファイルが
    static/ に存在する必要があります。
    ...
    2. npm install
    3. npm run build:win
    4. cp dist/*.exe ../static/printer-server-setup.exe
    5. zip -j static/printer-server-setup.zip static/printer-server-setup.exe
    """
    from flask import send_file, abort
    
    # static フォルダのパス
    static_dir = os.path.join(os.path.dirname(__file__), "static")
    zip_file = os.path.join(static_dir, "printer-server-setup.zip")
    
    if not os.path.exists(zip_file):
        # ZIPファイルが見つからない場合はエラー
        abort(404, description="ZIPファイルが見つかりません。ビルドを実行してください。")
    
    return send_file(
        zip_file,
        mimetype='application/zip',
        as_attachment=True,
        download_name="printer-server-setup.zip"
    )
...
# ========================================
@bp.route('/download')
def printer_server_download_page():
    """
    プリンターサーバーアプリのダウンロードページ
    """
    return render_template('pos/download.html')

# ========================================
# Printer-Server印刷ルールAPI
# ========================================
@bp.route('/api/printer-server/rules', methods=['GET'])
def get_print_rules():
    """
    印刷ルールとプリンタ情報を返す
    ...
    try:
        ...
        if not store_id or not api_key:
            return jsonify({"error": "store_id and api_key are required"}), 400
        
        session = SessionLocal()
        try:
            # APIキー検証（生SQLでテナントフィルタを回避）
            result = session.execute(
                text('SELECT id, "プリンターサーバーAPIキー" FROM "M_店舗" WHERE id = :store_id'),
                {"store_id": store_id}
            ).fetchone()
            
            logging.warning(f"[PRINTER-RULES-API] SQL result={result}, store_id={store_id}")
            
            if not result:
                logging.warning(f"[PRINTER-RULES-API] Store not found for store_id={store_id}")
                return jsonify({"error": "Store not found"}), 404
            
            store_id_db, expected_api_key = result
            logging.warning(f"[PRINTER-RULES-API] expected_api_key='{expected_api_key}'")
            logging.warning(f"[PRINTER-RULES-API] provided api_key='{api_key}'")
            logging.warning(f"[PRINTER-RULES-API] Match: {expected_api_key == api_key}")
            
            if expected_api_key != api_key:
                logging.warning(f"[PRINTER-RULES-API] Invalid API key for store_id={store_id}")
                return jsonify({"error": "Invalid API key"}), 403
            
            # プリンタ情報を取得（printer_server種別のみ）
            printers = session.query(Printer).filter(
                Printer.store_id == store_id,
                Printer.kind == 'printer_server',
                Printer.enabled == 1
            ).all()
            ...
            # 印刷ルールを取得
            rules = session.query(PrintRule).filter(
                PrintRule.store_id == store_id
            ).all()
            ...
            logging.info(f"[PRINTER-RULES-API] Returned {len(printer_list)} printers and {len(rule_list)} rules for store_id={store_id}")
        ...
        logging.error(f"[PRINTER-RULES-API] Error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500


# ========================================
# Printer-Server自動登録API
# ========================================
@bp.route('/api/printer-server/register-printers', methods=['POST'])
def register_printers_from_server():
    """
    Printer-Serverアプリから設定済みプリンタ情報を受信して、データベースに登録する
    ...
    try:
        ...
        printer_server_url = data.get('printer_server_url')  # プリンタサーバーのURL（例: http://localhost:3001）
        ...
        try:
            # APIキー検証（生SQLでテナントフィルタを回避）
            result = session.execute(
                text('SELECT id, "プリンターサーバーAPIキー", tenant_id FROM "M_店舗" WHERE id = :store_id'),
                {"store_id": store_id}
            ).fetchone()
            
            logging.warning(f"[REGISTER-PRINTERS] SQL result={result}, store_id={store_id}")
            
            if not result:
                logging.warning(f"[REGISTER-PRINTERS] Store not found for store_id={store_id}")
                return jsonify({"error": "Store not found"}), 404
            
            store_id_db, expected_api_key, store_tenant_id = result
            logging.warning(f"[REGISTER-PRINTERS] expected_api_key='{expected_api_key}'")
            logging.warning(f"[REGISTER-PRINTERS] provided api_key='{api_key}'")
            logging.warning(f"[REGISTER-PRINTERS] Match: {expected_api_key == api_key}")
            
            if expected_api_key != api_key:
                logging.warning(f"[REGISTER-PRINTERS] Invalid API key for store_id={store_id}")
                return jsonify({"error": "Invalid API key"}), 403
            
            # プリンタサーバーURLを店舗テーブルに保存
            if printer_server_url:
                store = session.get(Store, store_id)
                if store:
                    store.printer_server_url = printer_server_url
                    logging.warning(f"[REGISTER-PRINTERS] Updated printer_server_url: {printer_server_url}")
            
            registered_count = 0
            updated_count = 0
            
            logging.warning(f"[REGISTER-PRINTERS] Received {len(printers)} printers")
            logging.warning(f"[REGISTER-PRINTERS] Using store_id: {store_id}")
            logging.warning(f"[REGISTER-PRINTERS] Store tenant_id: {store_tenant_id}")
            logging.warning(f"[REGISTER-PRINTERS] Current tenant_id from _current_tenant_id(): {_current_tenant_id()}")
            logging.warning(f"[REGISTER-PRINTERS] Printers data: {printers}")
            logging.warning(f"[REGISTER-PRINTERS] Printer server URL: {printer_server_url}")
            
            # リクエストに含まれるプリンタ名のセット
            printer_names_in_request = {p.get('name') for p in printers}
            
            # 既存のprinter_serverプリンタを取得
            existing_printers = session.query(Printer).filter(
                Printer.store_id == store_id,
                Printer.kind == 'printer_server'
            ).all()
            ...
            # リクエストに含まれないプリンタを削除
            deleted_count = 0
            for existing in existing_printers:
                if existing.name not in printer_names_in_request:
                    # 印刷ルールで使用されているかチェック
                    from sqlalchemy import text as sql_text
                    rule_count = session.execute(
                        sql_text('SELECT COUNT(*) FROM "M_印刷ルール" WHERE "プリンタID" = :printer_id'),
                        {"printer_id": existing.id}
                    ).scalar()
                    
                    if rule_count > 0:
                        logging.warning(f"[REGISTER-PRINTERS] Cannot delete printer {existing.name} (id={existing.id}): used in {rule_count} print rules")
                    else:
                        logging.warning(f"[REGISTER-PRINTERS] Deleting printer: {existing.name} (id={existing.id})")
                        session.delete(existing)
                        deleted_count += 1
            
            # リクエストに含まれるプリンタを処理
            for p in printers:
                logging.warning(f"[REGISTER-PRINTERS] Processing printer: {p}")
                
                printer_name = p.get('name')
                printer_enabled = 1 if p.get('enabled') else 0
                connection_str = f"printer-server://{printer_name}"
                
                # 既存のプリンタを生SQLで検索（セッションキャッシュを回避）
                existing_rows = session.execute(
                    text('SELECT id FROM "M_プリンタ" WHERE "店舗ID" = :store_id AND "名称" = :name AND "種別" = :kind ORDER BY id'),
                    {"store_id": store_id, "name": printer_name, "kind": "printer_server"}
                ).fetchall()
                
                logging.warning(f"[REGISTER-PRINTERS] existing_rows for {printer_name}: {existing_rows}")
                
                if existing_rows:
                    # 最初の1件を更新、残りは重複削除
                    keep_id = existing_rows[0][0]
                    for dup_row in existing_rows[1:]:
                        dup_id = dup_row[0]
                        logging.warning(f"[REGISTER-PRINTERS] Deleting duplicate printer id={dup_id} ({printer_name})")
                        session.execute(
                            text('DELETE FROM "M_プリンタ" WHERE id = :id'),
                            {"id": dup_id}
                        )
                    # 既存レコードを更新
                    session.execute(
                        text('UPDATE "M_プリンタ" SET "種別" = :kind, "接続情報" = :connection, "有効" = :enabled, tenant_id = :tenant_id WHERE id = :id'),
                        {"kind": "printer_server", "connection": connection_str, "enabled": printer_enabled, "tenant_id": store_tenant_id, "id": keep_id}
                    )
                    updated_count += 1
                    logging.warning(f"[REGISTER-PRINTERS] Updated printer: {printer_name} (id={keep_id}, enabled={printer_enabled})")
                else:
                    # 新規登録
                    new_printer = Printer(
                        store_id=store_id,
                        tenant_id=store_tenant_id,  # tenant_idを明示的に設定
                        name=printer_name,
                        kind='printer_server',
                        connection=connection_str,
                        enabled=printer_enabled,  # enabled状態を反映
                        width=42
                    )
                    session.add(new_printer)
                    registered_count += 1
                    logging.warning(f"[REGISTER-PRINTERS] Registered new printer: {printer_name} (enabled={printer_enabled}, store_id={store_id}, tenant_id={store_tenant_id}, kind='printer_server')")
            
            session.commit()
            
            logging.warning(f"[REGISTER-PRINTERS] Final result: registered={registered_count}, updated={updated_count}, deleted={deleted_count}")
        ...
        logging.error(f"[REGISTER-PRINTERS] Error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500


# ========================================
# Printer-Server印刷設定取得API
# ========================================
@bp.route('/api/printer-server/print-settings/<printer_name>', methods=['GET'])
def get_printer_settings(printer_name):
    """
    プリンタの印刷設定を返す
    ...
    Query Parameters:
        store_id: 店舗ID
        api_key: APIキー
        order_id: 注文ID（オプション、指定時は注文データとKDS判定情報も返す）
    ...
    Response:
        {
            ...
            "settings": {
                ...
                "show_price": 1,
                "show_new_additional": 1,
                "kitchen_datetime_format": "HH:mm"
            },
            "order": {  // order_idが指定された場合のみ
                "id": 123,
                "table_name": "テーブル1",
    ...
    try:
        ...
        try:
            ...
            logging.info(f"[PRINT-SETTINGS-API] Checking store_id={store_id}")
            
            if not result:
                logging.warning(f"[PRINT-SETTINGS-API] Store not found for store_id={store_id}")
                return jsonify({"error": "Store not found"}), 404
            
            store_id_db, expected_api_key = result
            
            if expected_api_key != api_key:
                logging.warning(f"[PRINT-SETTINGS-API] Invalid API key for store_id={store_id}")
                return jsonify({"error": "Invalid API key"}), 403
            
            # プリンタ印刷設定を取得
            setting = session.query(PrinterFormatSetting).filter(
                PrinterFormatSetting.store_id == store_id,
            ...
            if not setting:
                ...
                settings_dict = {
                    ...
                    "margin_before": setting.margin_before,
                    "margin_after": setting.margin_after,
                    "show_title": setting.show_title,
                    "title_text": setting.title_text,
                    "title_size": setting.title_size,
                    "title_align": setting.title_align,
                    "title_bold": setting.title_bold,
                    "show_separator": setting.show_separator,
                    "show_datetime": setting.show_datetime,
                    "datetime_format": setting.datetime_format,
                    "show_price": setting.show_price,
                    "show_new_additional": setting.show_new_additional,
                    "kitchen_datetime_format": setting.kitchen_datetime_format
                }
            ...
            # order_idが指定された場合は注文データとKDS判定情報も返す
            if order_id:
                logging.info(f"[PRINT-SETTINGS-API] Querying order_id={order_id}, store_id={store_id}")
                
                # 生SQLで注文を検索（テナントスコープを回避）
                order_result = session.execute(
                    text('SELECT * FROM "T_注文" WHERE id = :order_id AND "店舗ID" = :store_id'),
                    {"order_id": order_id, "store_id": store_id}
                ).fetchone()
                
                logging.info(f"[PRINT-SETTINGS-API] Order query result: {order_result}")
                
                if not order_result:
                    logging.error(f"[PRINT-SETTINGS-API] Order not found: order_id={order_id}, store_id={store_id}")
                    return jsonify({"error": "Order not found"}), 404
                
                # OrderHeaderオブジェクトとして取得（テナントスコープを回避するため生SQLを使用）
                order_row = session.execute(
                    text('SELECT id, "テーブルID" as table_id, "開始日時" as opened_at FROM "T_注文" WHERE id = :order_id'),
                    {"order_id": order_id}
                ).fetchone()
                
                if not order_row:
                    logging.error(f"[PRINT-SETTINGS-API] Order not found in ORM: order_id={order_id}, store_id={store_id}")
                    return jsonify({"error": "Order not found"}), 404
                
                # order_rowをオブジェクト風に扱うためのラッパー
                class OrderProxy:
                    def __init__(self, row):
                        self.id = row[0]
                        self.table_id = row[1]
                        self.opened_at = row[2]
                        # created_atは互換性のためopened_atのエイリアス
                        self.created_at = row[2]
                order = OrderProxy(order_row)
                
                # 注文明細を取得（生SQLでテナントスコープを回避）
                order_details_rows = session.execute(
                    text('SELECT oi.id, oi."メニューID" as menu_id, oi."数量" as qty, oi."単価" as unit_price FROM "T_注文明細" oi WHERE oi."注文ID" = :order_id'),
                    {"order_id": order_id}
                ).fetchall()
                
                logging.info(f"[PRINT-SETTINGS-API] Order details count: {len(order_details_rows)}")
                
                items_list = []
                for detail_row in order_details_rows:
                    detail_id, menu_id, qty, unit_price = detail_row
                    
                    # メニュー情報を取得（生SQL） - カラム名は「名称」と「KDS判定グループ」
                    menu_row = session.execute(
                        text('SELECT id, "名称" as name, "KDS判定グループ" as kds_group FROM "M_メニュー" WHERE id = :menu_id'),
                        {"menu_id": menu_id}
                    ).fetchone()
                    
                    menu_name = menu_row[1] if menu_row else "不明"
                    kds_group = menu_row[2] if menu_row else None
                    ...
                    if kds_group:
                        prev_row = session.execute(
                            text('''
                                SELECT oh.id FROM "T_注文" oh
                                JOIN "T_注文明細" oi2 ON oi2."注文ID" = oh.id
                                JOIN "M_メニュー" m2 ON m2.id = oi2."メニューID"
                                WHERE oh."テーブルID" = :table_id
                                AND oh.id < :order_id
                                AND oh."状態" != '取消'
                                AND m2."KDS判定グループ" = :kds_group
                                LIMIT 1
                            '''),
                            {"table_id": order.table_id, "order_id": order_id, "kds_group": kds_group}
                        ).fetchone()
                        if prev_row:
                            is_new = False
                ...
                logging.info(f"[PRINT-SETTINGS-API] items_list count: {len(items_list)}")
                
                # テーブル情報を取得（生SQL） - テーブル名は「テーブル番号」カラム
                table_row = session.execute(
                    text('SELECT "テーブル番号" as table_no FROM "M_テーブル" WHERE id = :table_id'),
                    {"table_id": order.table_id}
                ).fetchone()
                table_name = table_row[0] if table_row else "不明"
                
                # 小計、税、合計を計算
                subtotal = sum(item['price'] * item['quantity'] for item in items_list)
                tax = int(subtotal * 0.1)  # 10%税率
                total = subtotal + tax
                
                response_data["order"] = {
                    "id": order.id,
                    "table_name": table_name,
                    "datetime": order.created_at.strftime("%Y-%m-%d %H:%M:%S") if order.created_at else "",
                    "items": items_list,
                    "subtotal": subtotal,
                    "tax": tax,
                    "total": total
            ...
            logging.info(f"[PRINT-SETTINGS-API] Returned settings for printer '{printer_name}' at store_id={store_id}")
        ...
        logging.error(f"[PRINT-SETTINGS-API] Error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500


# ========================================
# Printer-Serverエラーログ記録API
# ========================================
@bp.route('/api/printer-server/log-error', methods=['POST'])
def log_printer_server_error():
    """
    Printer-Serverアプリからエラーログを受信して記録する
    ...
    try:
        ...
        printer_name = data.get('printer_name', 'unknown')
        order_id = data.get('order_id')
        ...
        try:
            store_row = session.execute(
                text('SELECT id, "プリンターサーバーAPIキー" as api_key FROM "M_店舗" WHERE id = :store_id'),
                {"store_id": store_id}
            ).fetchone()
            if not store_row:
                logging.error(f"[PRINTER-ERROR-LOG] Store not found: {store_id}")
                return jsonify({"error": "Store not found"}), 404
            
            expected_api_key = store_row[1]
            if api_key != expected_api_key:
                logging.error(f"[PRINTER-ERROR-LOG] Invalid API key for store {store_id}")
                return jsonify({"error": "Invalid API key"}), 401
            
            # エラーログを記録
            logging.error(
                f"[PRINTER-SERVER-ERROR] "
                f"store_id={store_id}, "
                f"type={error_type}, "
            ...
            if stack_trace:
                logging.error(f"[PRINTER-SERVER-ERROR] Stack trace:\n{stack_trace}")
        ...
        logging.error(f"[PRINTER-ERROR-LOG] Error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500

@bp.route('/api/printer-server/log-info', methods=['POST'])
def log_printer_server_info():
    """
    Printer-Serverアプリから重要ログを受信して記録する
    
    Request Body:
        {
            "store_id": 1,
            "api_key": "...",
            "log_type": "print_success" | "polling_start" | "polling_stop" | "polling_error",
            ...追加データ
        }
    ...
    try:
        ...
        try:
            ...
            # ログタイプに応じて記録
            if log_type == 'print_success':
                printer_name = data.get('printer_name', 'unknown')
                print_mode = data.get('print_mode', 'unknown')
                order_id = data.get('order_id')
                table_name = data.get('table_name', '')
                logging.info(
                    f"[PRINTER-SERVER] 印刷成功 "
                    f"store_id={store_id}, printer={printer_name}, "
                    f"mode={print_mode}, order_id={order_id}, table={table_name}"
                )
            
            elif log_type == 'polling_start':
                interval_ms = data.get('interval_ms', 10000)
                printers = data.get('printers', [])
                logging.info(
                    f"[PRINTER-SERVER] ポーリング開始 "
                    f"store_id={store_id}, interval={interval_ms}ms, printers={printers}"
                )
            
            elif log_type == 'polling_stop':
                logging.info(f"[PRINTER-SERVER] ポーリング停止 store_id={store_id}")
            
            elif log_type == 'polling_error':
                error_message = data.get('error_message', '')
                logging.error(
                    f"[PRINTER-SERVER] ポーリングエラー "
                    f"store_id={store_id}, message={error_message}"
                )
            
            elif log_type == 'test_print_start':
                printer_name = data.get('printer_name', 'unknown')
                command_mode = data.get('command_mode', 'escpos')
                ip = data.get('ip', '')
                port = data.get('port', '')
                logging.info(
                    f"[PRINTER-SERVER] テスト印刷開始 "
                    f"store_id={store_id}, printer={printer_name}, mode={command_mode}, ip={ip}:{port}"
                )
            
            elif log_type == 'test_print_success':
                printer_name = data.get('printer_name', 'unknown')
                command_mode = data.get('command_mode', 'escpos')
                logging.info(
                    f"[PRINTER-SERVER] テスト印刷成功 "
                    f"store_id={store_id}, printer={printer_name}, mode={command_mode}"
                )
            
            elif log_type == 'test_print_error':
                printer_name = data.get('printer_name', 'unknown')
                command_mode = data.get('command_mode', 'escpos')
                error_message = data.get('error_message', '')
                logging.error(
                    f"[PRINTER-SERVER] テスト印刷失敗 "
                    f"store_id={store_id}, printer={printer_name}, mode={command_mode}, error={error_message}"
                )
            
            else:
                logging.info(f"[PRINTER-SERVER] {log_type} store_id={store_id} data={data}")
        ...
        logging.error(f"[PRINTER-INFO-LOG] Error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
