...
# ─── 認証ヘルパー ────────────────────────────────────────────
BREEDER_ROLES = (ROLES["SYSTEM_ADMIN"], ROLES["TENANT_ADMIN"], ROLES["APP_MANAGER"], ROLES["ADMIN"], ROLES["EMPLOYEE"])
...
def dashboard():
    # テナント管理者・アプリ管理者・システム管理者はテナント集計ページへ
    role = session.get('role')
    if role in (ROLES['SYSTEM_ADMIN'], ROLES['TENANT_ADMIN'], ROLES['APP_MANAGER']):
        return redirect(url_for('breeder.tenant_summary'))
...
def tenant_summary():
    ...
    from app.models_breeder import Dog, Puppy, Birth, Negotiation, Heat, Mating, PedigreeApplication
    ...
    try:
        ...
        if not tenant_id and is_system_admin:
            ...
            tenants = db.query(TTenant).filter(TTenant.id == tenant_id, TTenant.有効 == 1).all()
            stores = db.query(TTenpo).filter(TTenpo.tenant_id == tenant_id, TTenpo.有効 == 1).order_by(TTenpo.id).all()
        else:
            flash('テナント情報が見つかりません。', 'error')
            return redirect(url_for('breeder.dashboard'))
        ...
        for store in stores:
            ...
            parents = db.query(func.count(Dog.id)).filter(
                Dog.store_id == sid, Dog.status == 'active', Dog.dog_type == 'parent'
            ).scalar() or 0
            puppies = db.query(func.count(Puppy.id)).filter(
                Puppy.store_id == sid, Puppy.status.in_(['available', 'reserved'])
            ).scalar() or 0
            ...
            pregnant = db.query(func.count(Mating.id)).filter(
                Mating.store_id == sid, Mating.status == 'pregnant'
            ).scalar() or 0
            heat_active = db.query(func.count(Heat.id)).filter(
                Heat.store_id == sid, Heat.status.in_(['active', 'imminent'])
            ).scalar() or 0
            births_month = db.query(func.count(Birth.id)).filter(
                Birth.store_id == sid,
                func.extract('year', Birth.birth_date) == today.year,
                func.extract('month', Birth.birth_date) == today.month
            ).scalar() or 0
            # 店舗の最近商談（直近5件）
            recent_negs = db.query(Negotiation).filter(
                Negotiation.store_id == sid
            ...
            alert_count = 0
            try:
                recent_births = db.query(Birth).filter(
                    Birth.store_id == sid, Birth.birth_date >= cutoff_70
                ).all()
                for birth in recent_births:
                    for puppy in db.query(Puppy).filter(Puppy.birth_id == birth.id).all():
                        if not db.query(PedigreeApplication).filter(
                            PedigreeApplication.puppy_id == puppy.id
                        ).first():
                            alert_count += 1
            except Exception:
            ...
            store_stats.append({
                ...
                'pregnant': pregnant,
                'heat_active': heat_active,
                'births_month': births_month,
                'recent_negotiations': recent_negs,
                'alert_count': alert_count,
            })

        # 全体サマリー（店舗別集計の合計）
        total_parents = sum(s['parents'] for s in store_stats)
        total_puppies = sum(s['puppies'] for s in store_stats)
        total_negotiations = sum(s['negotiations'] for s in store_stats)
        total_pregnant = sum(s['pregnant'] for s in store_stats)
        total_heat_active = sum(s['heat_active'] for s in store_stats)
        total_births_month = sum(s['births_month'] for s in store_stats)
        unregistered_alert_count = sum(s['alert_count'] for s in store_stats)
        ...
        return render_template('breeder/tenant_summary.html',
            ...
            total_births_month=total_births_month,
            total_heat_active=total_heat_active,
            birth_stats=birth_stats,
            unregistered_alert_count=unregistered_alert_count,
            today=today,
        )
