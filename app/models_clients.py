...
class TClient(Base):
    ...
    # e-Tax認証情報（暗証番号は保管時に暗号化: EncryptedString）
    etax_user_id = Column(String(100), nullable=True)    # e-Tax 利用者識別番号
    etax_password = Column(EncryptedString(255), nullable=True)   # e-Tax パスワード（暗号化保管）

    # eLTAX認証情報（暗証番号は保管時に暗号化: EncryptedString）
    eltax_user_id = Column(String(100), nullable=True)   # eLTAX 利用者ID
    eltax_password = Column(EncryptedString(255), nullable=True)  # eLTAX パスワード（暗号化保管）
