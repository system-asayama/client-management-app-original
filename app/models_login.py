...
class TKanrisha(Base):
    ...
    can_manage_tenants = Column(Integer, default=0, comment='テナント管理権限（1=テナントの作成・編集・管理が可能）')
    can_manage_all_tenants = Column(Integer, default=0, comment='全テナント管理権限（1=全テナントにアクセス可能、0=作成/招待されたテナントのみ）')
    can_manage_api_keys = Column(Integer, default=0, comment='API設定権限（1=APIキーの設定・変更が可能）')
    can_manage_app_managers = Column(Integer, default=0, comment='アプリ管理者管理権限（1=アプリ管理者の作成・編集・削除が可能）')
...
class TTenant(Base):
    ...
    azure_document_intelligence_endpoint = Column(Text, nullable=True, comment='Azure Document Intelligence エンドポイントURL')
    azure_document_intelligence_key = Column(Text, nullable=True, comment='Azure Document Intelligence APIキー')
    ai_model = Column(String(50), default='gpt-4o', comment='AIモデル選択')
    ...
    app_manager_group_id = Column(Integer, ForeignKey('T_アプリ管理者グループ.id'), nullable=True, comment='このテナントを作成したアプリ管理者グループID')
...
class TTenpo(Base):
    ...
    ai_model = Column(String(50), default='gpt-4o', comment='AIモデル選択')
    # SMTPメール送信設定（店舗単位）
    smtp_host = Column(String(255), nullable=True, comment='SMTPサーバーホスト')
    smtp_port = Column(Integer, nullable=True, comment='SMTPポート')
    smtp_username = Column(String(255), nullable=True, comment='SMTPユーザー名')
    smtp_password = Column(Text, nullable=True, comment='SMTPパスワード')
