...
# 【安全装置】本番へのログイン疎通テストの実行可否。
# ログインPOSTが認証画面(SU00S010)へ戻る＝認証失敗の可能性があり、
# 繰り返すとe-Taxアカウントがロックされる恐れがあるため、既定で False（停止）。
# 暗証番号の正しさ確認、または送信試験環境の準備ができてから True にする。
ALLOW_LIVE_LOGIN_PROBE = False
...
class EtaxWebClient:
    ...
    def login_probe(user_id: str, password: str) -> Dict[str, Any]:
        ...
        """
        # 安全装置: アカウントロック回避のため、既定では本番ログインを実行しない。
        if not ALLOW_LIVE_LOGIN_PROBE:
            return {"ok": False, "trace": [], "boot": None,
                    "error": "【安全のため一時停止中】本番へのログイン試行は停止しています"
                             "（認証失敗を繰り返すとe-Taxアカウントがロックされる恐れがあるため）。"
                             "顧問先本人がe-Taxに手動ログインできる正しい暗証番号かをご確認いただくか、"
                             "送信試験環境の準備後に再開します。"}
