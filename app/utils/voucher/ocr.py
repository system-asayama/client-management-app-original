...
【摘要の読み取りルール】
- 摘要列には印字（活字）部分と手書き部分が混在している場合がある
- 印字（活字）部分のみを description に入れる
  例: 「カード〃(植松 ノートパソコン)」→ description=「カード」
  例: 「デントウ - 3カツ」→ description=「デントウ」
  例: 「オカヤマケンコクミンケンコ」→ description=「オカヤマケンコクミンケンコ」（全て印字の場合はそのまま）
  例: 「タムラ カズエ」→ description=「タムラ カズエ」（人名は全てそのまま）
- 手書き部分（カッコ内の注記・補足など）は note に入れる
  例: 「カード〃(植松 ノートパソコン)」→ note=「植松 ノートパソコン」
  例: 「カード〃(浅山さん3月給料)」→ note=「浅山さん3月給料」
  例: 「カード〃(電気代3月 通川)」→ note=「電気代3月 通川」
- 「〃」「″」「'」は「同上」を意味する略記符号（直前の摘要と同じ意味）
- 備考欄（最右列の手数記号980・960・186など）も note に追記する
- 摘要列と備考列（最右列）を混同しないこと
