...
【通帳の列構造について】
日本の銀行通帳の列は左から次の順で並んでいることが多い：
  「年月日」 | 「記号」 | 「お払戈し金額（出金）」 | 「お預り金額／お利息（入金）」 | 「差引残高」 | 「備考」

重要：「記号」列（100、900などの数字）は取引種別コードであり、金額ではない。入金・出金に含めないこと。

【入金（deposit）・出金（withdrawal）の判定ルール】
- 「お払戈し」「お払い」「お引き出し」「出金」「お払戈し金額」の列にある金額 = withdrawal（出金）
- 「お預り」「お頂き」「入金」「お利息」「お預り金額」の列にある金額 = deposit（入金）
- 各行は必ず入金または出金のどちらか一方のみに数値を入れ、もう一方はnullにする
- 「*」の付いた金額は正規の金額（「*」は除去して数値のみ返す）
- 列の位置で判断する：左側の金額列（お払戈し）は出金、右側の金額列（お預り）は入金

【日付の解釈ルール】
- 通帳の日付は「年月日」形式。例: "08-03-16" = 2008年3月16日
- 2桁の年は西暦に変換。"00"-"29" → 2000-2029年、"30"-"99" → 1930-1999年
...
{{
  ...
  "transactions": [
    {{
      ...
      "deposit": "入金額（お預り金額列の値、数値のみ、出金の場合はnull）",
      "withdrawal": "出金額（お払戈し金額列の値、数値のみ、入金の場合はnull）",
